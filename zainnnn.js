...
function load() {
	...
	}, alert(S)), $('#tlogins button').attr('disabled', 'true'), processq(), newsock(), '' == getv('refr') && setv('refr', refr() || '*'), '' == getv('r') && setv('r', getUrlParameter('r') || '*'), $(window).on('resize pushclose pushopen', fixSize), $('*[data-toggle="tab"]').on('shown.bs.tab', function() {
		fixSize()
...
function processq() {
	for (var C = 0, S; C < onq.length && 20 > C; C++) S = onq[0], onq.splice(0, 1), ondata(S.cmd, S.data);
	setTimeout(function() {
		processq()
	}, 100)
}
...
function fixI(C) {
	if (!1 != isIphone) {
		...
		S < document.body.scrollHeight + window.innerHeight, $(document.body).scrollTop(S)
	}
}
...
function refreshonline() {
	$.get('getonline', function(C) {
		var S = C;
		'stromg' == typeof C && JSON.parse(C), powers = S.powers;
		var E = $('.lonline');
		E.children().remove();
		var P = $('#uhtml').html();
		$('.s1').text(S.online.length), $.each(S.online, function(R, D) {
			if (!0 != D.s) {
				var M = $(P);
				M.find('.u-topic').text(D.topic).css({
					'background-color': D.bg,
					color: D.ucol
				}), M.find('.u-msg').text(D.msg), M.find('.u-pic').css('background-image', 'url("' + D.pic + '")'), M.find('.ustat').remove(), '--' == D.co || null == D.co || 'A1' == D.co || 'A2' == D.co || 'EU' == D.co ? M.find('.co').remove() : M.find('.co').attr('src', 'https://iqr30.com/sico/z1cki553su10.gif');
				var A = getico(D);
				'' != A && M.find('.u-ico').attr('src', A), E.append(M)
			}
		})
	})
}
...
function jsonhtml(C, S) {
	...
	return $.each(Object.keys(C), function(P, R) {
		switch (typeof C[R]) {
			case 'string':
				E.append('<label class="label label-primary">' + R + '</label></br>'), E.append('<input type="text" name="' + R + '" class="corner" value="' + C[R] + '"></br>');
				break;
			case 'boolean':
				E.append('<label class="label label-primary">' + R + '</label></br>');
				var D = '';
				C[R] && (D = 'checked'), E.append('<label>\u062A\u0641\u0639\u064A\u0644<input name="' + R + '" type="checkbox" class="corner" ' + D + '></label></br>');
				break;
			case 'number':
				E.append('<label class="label label-primary">' + R + '</label></br>'), E.append('<input name="' + R + '" type="number" class="corner" value="' + C[R] + '"></br>');
		}
	}), E.append('<button class="btn btn-primary fr fa fa-edit">\u062D\u0641\u0638</button>'), E.find('button').click(function() {
		S(htmljson(E))
	}), E
}
var lastfix = 0,
...
function fixSize(C) {
	...
	var E = $('.lonline');
	0 < E.length && E.css('height', $(window).height() - E.position().top - 5 + 'px'), $('.dpnl').css('left', $('.dad').width() - ($('.dpnl').width() + 2) + 'px').css('height', $('#room').height() - ($('#d0').height() + 2) + 'px').css('top', '0px'), 1 == C ? $('#d2').scrollTop($('#d2')[0].scrollHeight) : setTimeout(function() {
		fixSize(1)
	}, 10)
}
...
function startcss() {
	$.each($('.tab-pane'), function(C, S) {
		...
		$($(C.relatedTarget).attr('href')).addClass('hid'), $($(C.target).attr('href')).removeClass('hid')
	})
}
...
function pickedemo(C) {
	...
	var S = C.attr('title'),
		E = $(C.attr('eid'));
	E.parent().find('.tbox').val($(E).parent().find('.tbox').val() + ' \u0641' + S), E.popover('hide').blur()
}
...
function roomChanged(C) {
	...
	if ($('.bord').removeClass('bord'), null != S) {
		$('.inr,.ninr,.rout').show(), 0 == $('#room.active').length && !0 == C && $('[data-target=\'#room\']').trigger('click'), !0 == C && $('[data-target=\'#room\']').show(), $.each(rusers(S.id), function() {
			$('#users').find('.uid' + this.id).addClass('inroom')
		}), $('#rooms').find('.r' + S.id).addClass('inroom bord'), $('#tbox').css('background-color', '');
		var E = getuser(myid);
		E && (S.owner == E.lid || !0 == power.roomowner) && $('.redit').show()
	} else $('.roomtgl').hide(), C && $('[data-target=\'#room\']').hide(), 0 != $('#room.active').length && !0 == C && $('[data-target=\'#rooms\']').trigger('click'), $('.inr,.ninr').hide(), $('.rout').hide(), $('.redit').hide(), $('#tbox').css('background-color', '#AAAAAF')
}
	...
	try {
		switch (C) {
			...
			case 'dro3':
				...
				break;
			case 'emos':
				emos = S, emopop('.emobox'), emopop('.emobc');
				break;
			case 'login':
				switch ($('#tlogins button').removeAttr('disabled'), S.msg) {
					case 'ok':
						mylat = S.lat, myid = S.id, token = S.ttoken, setv('token', token), window.onbeforeunload = confirmOnPageExit, $('.dad').css('max-width', '100%'), $('#tlogins,.lonline').remove(), $('#d2,.footer,#d0').show(), fixSize();
						break;
					case 'noname':
			...
			case 'powers':
				powers = S;
				for (var E = 0, P; E < powers.length; E++) P = powers[E].name, '' == P && (P = '_'), powers[P] = powers[E];
				var R = getuser(myid);
				null != R && (power = getpower(R.power || ''), power.cp ? $('.cp').show() : $('.cp').hide(), 0 < power.publicmsg ? $('.pmsg').show() : $('.pmsg').hide()), $.each(users, function(H, J) {
					updateu(J.id, J)
				});
				break;
			case 'rops':
				var D = getroom(getuser(myid).roomid);
				D.ops = [], $.each(S, function(H, J) {
					D.ops.push(J.lid)
				});
				break;
			case 'power':
				power = S, power.cp ? $('.cp').show() : $('.cp').hide(), 0 < power.publicmsg ? $('.pmsg').show() : $('.pmsg').hide(), $.each(users, function(H, J) {
					updateu(J.id, J)
				});
				break;
			case 'not':
				...
				if (null != A) {
					...
					I.append('<img src=\'' + A.pic + '\' style=\'width:24px;height:24px;\' class=\'corner borderg fl\'>'), I.append('<img class=\'u-ico fl \' style=\'max-height:18px;\' > <div   style=\'max-width:80%;\' class=\'dots corner u-topic fl\'>' + A.topic + '</div>'), I.find('.u-topic').css({
						'background-color': A.bg,
						color: A.ucol
					});
					var B = getico(A);
					'' != B && I.find('.u-ico').attr('src', B), M.append(I)
				}
				M.append('<div   style=\'width:100%;display:block;padding:0px 5px;\' class=\'break fl\'>' + emo(S.msg) + '</div>'), M.css('margin-left', '+=' + notpos), notpos += 2, 6 <= notpos && (notpos = 0), $('.dad').append(M);
				break;
			case 'delbc':
				$('.bid' + S.bid).remove();
				break;
			...
			case 'ops':
				var U = $('#ops');
				U.children().remove(), $.each(S, function(H, J) {
					var W = $($('#uhead').html()).css('background-color', 'white');
					W.find('.u-pic').css('width', '24px').css('height', '24px').css('background-image', 'url("' + J.pic + '")'), W.find('.u-topic').html(J.topic), W.find('.filw').removeClass('filw').css('width', '80%'), W.append('<a onclick="send(\'op-\',{lid: \'' + J.lid + '\'});" class="fa fa-times">\u0625\u0632\u0627\u0644\u0647</a>'), U.append(W)
				});
				break;
			case 'pm':
				...
				openw(S.pm, !1), AddMsg('#d2' + S.pm, S), $('#c' + S.pm).find('.u-msg').text(gettext($('<div>' + S.msg + '</div>'))), $('#c' + S.pm).insertAfter('#chats .chatsh');
				break;
			case 'pmsg':
				S.class = 'pmsgc';
				var F = AddMsg('#d2', S);
				F.find('.u-msg').append('<label style="margin-top:2px;color:blue" class="fl nosel fa fa-commenting">\u0625\u0639\u0644\u0627\u0646</label>'), 0 == $('#room.active').length && hl($('[data-target=\'#room\']'), 'warning');
				break;
			case 'msg':
				AddMsg('#d2', S), 0 == $('#room.active').length && hl($('[data-target=\'#room\']'), 'warning');
				break;
			case 'close':
				close();
				break;
			case 'ulist':
				users = S, $('.busers').text($.grep(users, function(H) {
					return null == H.s
				}).length), $.each(users, function(H, J) {
					AddUser(J.id, J)
				});
				break;
			case 'u-':
				$('.uid' + S).remove(), users = $.grep(users, function(H) {
					return H.id != S
				}), wclose(S), $('.busers').text($.grep(users, function(H) {
					return null == H.s
				}).length);
				break;
			case 'u+':
				users.push(S), AddUser(S.id, S), updateu(S.id, S), $('.busers').text($.grep(users, function(H) {
					return null == H.s
				}).length);
				break;
			case 'ur':
				var G = S[0],
					...
					D = getroom(z),
					R = getuser(G);
				G == myid && (myroom = z), null != R && (R.roomid = z, needUpdate = !0, roomChanged(G == myid));
				break;
			case 'u^':
				if (null == users) return;
				users = $.grep(users, function(H) {
					return H.id != S.id
				}), users.push(S), updateu(S.id, S), needUpdate = !0;
				break;
			case 'r^':
				S.id == myroom && (S.ops = getroom(myroom).ops), rooms = $.grep(rooms, function(H) {
					return H.id != S.id
				}), rooms.push(S), updater(S);
				break;
			case 'rlist':
				rooms = S, $.each(rooms, function(H, J) {
					addroom(J)
				});
				break;
			case 'r+':
				rooms.push(S), addroom(S);
				break;
			case 'r-':
				$('.r' + S.id).remove(), rooms = $.grep(rooms, function(H) {
					return H.id != S.id
				});
				break;
			case 'r^':
			...
			case 'calling':
				...
				if (0 == $('#c' + S.caller).length) return send('nopm', {
					id: S.caller
				}), send('calldeny', S), void(wr && wr.hangUp());
				if (null == wr && 0 == $('.callnot').length && null != V && 0 < $('#d2' + S.caller).length) {
					var q = $($('#callnot').html()),
						I = $($('#uhtml').html());
					I.find('.u-msg').remove(), I.find('.u-topic').html(V.topic).css({
						color: V.ucol,
						'background-color': V.bg
					}), I.find('.u-pic').css('background-image', 'url("' + V.pic + '")').css({
						width: '24px',
						height: '24px'
					}), q.find('.uzer').append(I), q.addClass('callnot'), callid = S.caller, q.attr('callid', S.roomid), q.find('.calldeny').click(function() {
						q.remove(), send('calldeny', S), wr && wr.hangUp()
					}), q.find('.callaccept').click(function() {
						callstat = 1, $(document.body).append(q), wr = new webrtc(S.roomid, myid), $(this).hide()
					}), $('#d2' + S.caller).append(q), hl($('.callstat').text(''), 'warning'), updateu(V.id), openw(S.pm, !1)
...
function call(C) {
	...
	if (0 == callstat && null == wr && 0 == $('.callnot').length && null != S) {
		...
		P.find('.u-topic').html(S.topic).css({
			...
			width: '24px',
			height: '24px'
		}), E.find('.uzer').append(P), E.addClass('callnot'), E.attr('callid', R), E.find('.callaccept').hide(), E.find('.calldeny').click(function() {
			E.remove(), send('calldeny', {
				caller: myid,
				called: C,
				roomid: R
			}), wr && wr.hangUp()
		}), $(document.body).append(E), updateu(S.id), send('calling', {
			caller: myid,
			called: C,
			roomid: R
...
function gettext(C) {
	return $.each(C.find('img'), function(S, E) {
		var P = $(E).attr('alt');
		null != P && $('<x>' + P + '</x>').insertAfter($(E)), $(E).remove()
	}), $(C).text()
}
...
function hl(C, S) {
	C = $(C);
	var E = '';
	return C.hasClass('label') && (E = 'label'), C.hasClass('btn') && (E = 'btn'), C.hasClass('panel') && (E = 'panel'), $(C).removeClass(E + '-primary ' + E + '-danger ' + E + '-warning ' + E + '-info ' + E + '-success '), C.addClass(E + '-' + S), C
}
...
function setprofile() {
	var C = {
		...
		S = getuser(myid);
	C.pic = S.pic, C.username = S.username, setv('uprofile', JSON.stringify(C)), send('setprofile', C)
}
...
function updateu(C, S) {
	...
	if (null != E) {
		...
		E.s && (R = 'imgs/s4.png?2'), E.id == myid && ($('.spic').css('background-image', 'url("' + E.pic + '")'), $('.stopic').val(gettext($('<div>' + E.topic + '</div>'))), $('.smsg').val(gettext($('<div>' + E.msg + '</div>'))), $('.scolor').val(E.ucol).css('background-color', E.ucol).trigger('change'), $('.mcolor').val(E.mcol || '#000').css('background-color', E.mcol || '#000'), $('.sbg').val(E.bg).css('background-color', E.bg)), '' == E.msg && (E.msg = '..');
		var D = $('.uid' + C);
		if (D.find('.ustat').attr('src', R), '--' == E.co || null == E.co || 'A1' == E.co || 'A2' == E.co || 'EU' == E.co ? D.find('.co').remove() : D.find('.co').attr('src', 'flag/' + E.co.toLowerCase() + '.gif'), ismuted(E) ? D.find('.muted').toggleClass('fa-ban', !0) : D.find('.muted').toggleClass('fa-ban', !1), D.attr('v', getpower(E.power).rank), '' == P ? D.find('.u-ico').removeAttr('src') : D.find('.u-ico').attr('src', P), D.find('.u-topic').html(E.topic).css({
			...
			'' != P && D.find('.u-ico').attr('src', P), D.find('.ustat').attr('src', R), D.find('.u-topic').html(E.topic).css({
				'background-color': E.bg,
				color: E.ucol
			}), D.find('.u-pic').css('background-image', 'url("' + E.pic + '")'), D = $('.w' + C).find('.head .uzr'), D.find('.ustat').attr('src', R), '' != P && D.find('.u-ico').attr('src', P);
			var M = E.bg;
			'' == M && (M = '#FAFAFA'), D.find('.u-topic').html(E.topic).css({
				'background-color': M,
				color: E.ucol
			}), D.find('.u-pic').css('background-image', 'url("' + E.pic + '")')
		}
		return stealthit(E), void(needUpdate = !0)
	}
}
var needUpdate = !1,
	lastus = '';
...
function updateusers() {
	!1 == needUpdate || ($('#users').find('.uzr').sort(function(C, S) {
		...
		return $(C).hasClass('inroom') && (E += 1e5), $(S).hasClass('inroom') && (P += 1e5), $(C).hasClass('inr') && (E += 2e5), $(S).hasClass('inr') && (P += 2e5), $(C).hasClass('ninr') && (E += 9e3), $(S).hasClass('ninr') && (P += 9e3), E == P ? ($(C).find('.u-topic').text() + '').localeCompare($(S).find('.u-topic').text() + '') : E < P ? 1 : -1
...
function sendpm(C) {
	...
	$('.tbox' + C.data.uid).val(''), $('.tbox' + C.data.uid).focus();
	'%0A' == S || '%0a' == S || '' == S || '\n' == S || send('pm', {
		msg: S,
		id: C.data.uid
	})
...
function getico(C) {
	...
	return S = (getpower(C.power) || {
		ico: ''
	}).ico, '' != S && (S = 'sico/' + S), '' == S && '' != (C.ico || '') && (S = 'dro3/' + C.ico), S
}
...
function AddUser(C, S) {
	...
	if (!$('.uid' + C).length) {
		var P = getico(S);
		'' != P && E.find('.u-ico').attr('src', P), E.addClass('uid' + C), E.addClass('hid'), E.click(function() {
			upro(S.id)
		}), $('#users').append(E)
	}
}
...
function ytube(C, S) {
	$('<iframe width="95%" style="max-width:240px;" height="200" src="' + C + '" frameborder="0" allowfullscreen></iframe>').insertAfter($(S)), $(S).remove()
}
...
function AddMsg(C, S) {
	...
	E.find('.u-pic').css('background-image', 'url("' + S.pic + '")').click(function() {
		upro(S.uid)
	}), E.find('.u-topic').html(S.topic).css('color', S.ucol), S.msg = emo(S.msg);
		...
		$.ajax({
			...
			success: function(N, U, F) {
				if (F.getResponseHeader('Content-Type').match(/image/i)) {
					...
					G.insertAfter(I), $(I).remove(), G.find('button').click(function() {
						G.children().remove(), $('</br><a href=\'' + B + '\' target=\'_blank\'><img style=\'max-width:240px;max-height:200px;\' src=\'' + B + '\' class=\'hand fitimg\'></a>').insertAfter(G), G.remove()
					})
				}
				if (F.getResponseHeader('Content-Type').match(/video/i)) {
					...
					G.insertAfter(I), $(I).remove(), G.find('button').click(function() {
						G.children().remove(), $('<video style=\'width:95%;max-height:200px;\' controls><source src=\'' + B + '\'></video>').insertAfter(G), G.remove()
					})
				}
				if (F.getResponseHeader('Content-Type').match(/audio/i)) {
					...
					G.insertAfter(I), $(I).remove(), G.find('button').click(function() {
						G.children().remove(), $('<audio style=\'width:95%;\' controls><source src=\'' + B + '\' type=\'audio/mpeg\'></audio>').insertAfter(G), G.remove()
					})
				}
			}
		})
	}), !0 == M ? (100 <= $(C).find('.mm').length && $(C + ' .mm').last().remove(), $(C).scrollTop(0)) : (30 <= $(C).find('.mm').length && $(C + ' .mm').first().remove(), $(C).scrollTop($(C)[0].scrollHeight)), E
}
var isclose = !1;
...
function loadblocked() {
	...
	if (null != C && '' != C) try {
		C = JSON.parse(C), Array.isArray(C) && (blocked = C)
	} catch (S) {}
}
...
function upro(C) {
	...
	if (null != E && !(E.s && getpower(E.power).rank > power.rank)) {
		var P = $('#upro'),
			R = E.pic.split('.');
		2 < E.pic.split('/').pop().split('.').length && R.splice(R.length - 1, 1), P.find('.u-pic').css('background-image', 'url("' + R.join('.') + '")').removeClass('fitimg').addClass('fitimg'), P.find('.u-msg').html(E.msg), null != uf[(E.co || '').toLocaleLowerCase()] && P.find('.u-co').text(uf[E.co.toLocaleLowerCase()]).append('<img class="fl" src="flag/' + E.co.toLowerCase() + '.gif">');
		var D = getico(E),
			M = '\u0628\u062F\u0648\u0646 \u063A\u0631\u0641\u0647',
			A = getroom(E.roomid);
		if (!0 == power.unick || !0 == power.mynick && C == myid ? ($('.u-topic').val(E.topic), P.find('.nickbox').show(), P.find('.u-nickc').off().click(function() {
			...
			I.empty(), I.append('<option></option>');
			for (var B = 0; B < powers.length; B++)
				if (!(powers[B].rank > power.rank)) {
					var N = $('<option></option>');
					N.attr('value', powers[B].name), N.text(powers[B].name), I.append(N)
				}
			P.find('.powerbox .userdays').val(0), P.find('.upower').off().click(function() {
		...
		null == A ? P.find('.u-room').hide() : (null != A.ops && (-1 != A.ops.indexOf(getuser(myid).lid) || A.owner == getuser(myid).lid || power.roomowner) && (S = !0), M = '<div class="fl btn btn-primary dots roomh border" style="padding:0px 5px;max-width:180px;" onclick="rjoin(\'' + A.id + '\')"><img style="max-width:24px;" src=\'' + A.pic + '\'>' + A.topic + '</div>', P.find('.u-room').html(M), P.find('.u-room').show()), S ? P.find('.urkick,.umod').show() : P.find('.urkick,.umod').hide(), ismuted(E) ? (P.find('.umute').hide(), P.find('.uunmute').show()) : (P.find('.umute').show(), P.find('.uunmute').hide()), P.find('.ureport').hide(), !0 == power.history ? P.find('.uh').show() : P.find('.uh').hide(), 1 > power.kick ? (P.find('.ukick').hide(), P.find('.udelpic').hide()) : (P.find('.ukick').show(), P.find('.udelpic').show()), power.ban ? P.find('.uban').show() : P.find('.uban').hide(), 1 > power.upgrades ? P.find('.ugift').hide() : P.find('.ugift').show(), P.find('.uh').css('background-color', '').off().click(function() {
			$(this).css('background-color', 'indianred'), P.modal('hide');
			var F = $('<div style="height:100%;" class="u-div break light"></div>');
			popdiv(F, '\u062A\u0627\u0631\u064A\u062E \u062D\u064A\u0627\u062A\u0629'), $.get('uh?token=' + token + '&u2=' + C, function(G) {
				'object' == typeof G ? $.each(G, function(z, V) {
					var q = $('<div class=\'borderg\'></div>');
					q.append('<label class="label label-info">\u0627\u0633\u0645\u0629</lable><br>'), q.append($('<div></div>').text(V.username)), q.append('<label class="label label-info">\u0632\u062E\u0631\u0641\u062A\u0647</lable><br>'), q.append($('<div></div>').text(V.topic)), q.append('<label class="label label-info">\u0627\u0644\u0622\u064A \u0628\u064A</lable><br>'), q.append($('<div></div>').text(V.ip)), q.append('<label class="label label-info">\u062C\u0647\u0627\u0632\u0647</lable><br>'), q.append($('<div></div>').text(V.fp)), F.append(q)
				}) : F.text(G)
			})
		}), P.find('.umute').css('background-color', '').off().click(function() {
			$(this).css('background-color', 'indianred'), muteit(E), P.find('.umute').hide(), P.find('.uunmute').show()
		}), P.find('.uunmute').css('background-color', '').off().click(function() {
			$(this).css('background-color', 'indianred'), unmute(E), P.find('.umute').show(), P.find('.uunmute').hide()
		...
		}).text((E.rep || 0) + ''), P.find('.ureport').css('background-color', '').off().click(function() {
			$(this).css('background-color', 'indianred'), send('action', {
				cmd: 'report',
				id: C
			})
			...
			}), F.append('<button style=\'padding:5px;margin:4px;\' class=\'btn btn-primary hand borderg corner fa fa-ban\'  onclick=\'gift("' + C + '","");$(this).parent().pop("remove")\'>\u0625\u0632\u0627\u0644\u0647 \u0627\u0644\u0647\u062F\u064A\u0647</button>'), P.find('.ugift').popover({
				placment: 'left',
				content: F[0].outerHTML + '',
				trigger: 'focus',
				title: '\u0623\u0631\u0633\u0644 \u0647\u062F\u064A\u0647 !',
				html: !0
			}).popover('show'), $('.popover-content').html(F[0].outerHTML)
		}), P.modal({
			backdrop: 'static'
		});
		var U = '';
		'' != D && (U = '<img class="fl u-ico"  alt="" src="' + D + '">'), P.find('.modal-title').html('<img style=\'width:18px;height:18px;\' src=\'' + E.pic + '\'>' + U + E.topic), P.find('.upm').off().click(function() {
			P.modal('hide'), openw(C, !0)
		})
	}
}

function popframe(C, S) {
	$('#uh').length && $('#uh').parent().parent().remove(), newpop(S, '<iframe class=\'filh\' style=\'overflow: scroll !important;width:100%;height:100%;border:0px;\' id=\'uh\' src=\'' + C + '\'></iframe>')
}

function popdiv(C, S) {
	$('#uh').length && $('#uh').parent().parent().remove(), newpop(S, C)
}

function newpop(C, S) {
	var E = $($('#pop').html());
	return E.find('.title').append(C), E.find('.pphide').addClass('phide'), E.find('.body').append(S), $('.dad').append(E), E.show(), E
}
...
function mkr() {
	$('#ops').children().remove();
	var C = $('#mkr');
	C.find('.rsave').hide(), C.find('.rdelete').hide(), C.find('.modal-title').text('\u0645\u0645\u0646\u0648\u0639 \u0641\u062A\u062D \u0631\u0648\u0645 \u0634\u062E\u0635\u064A'), C.modal({
		backdrop: 'static'
	}), C.find('.rtopic').val(''), C.find('.rabout').val(''), C.find('.rpwd').val(''), C.find('.rwelcome').val(''), C.find('.rmax').val(''), C.find('.rdel').prop('checked', !1).parent().show(), C.find('.rmake').show().off().click(function() {
		send('r+', {
...
function redit(C) {
	$('#ops').children().remove(), null == C && (C = myroom);
	var S = getroom(C);
	if (null != S) {
		var E = $('#mkr');
		E.find('.modal-title').text('\u0625\u062F\u0627\u0631\u0647 \u0627\u0644\u063A\u0631\u0641\u0647'), E.find('.rsave').show().off().click(function() {
			...
			backdrop: 'static',
			title: 'ffff'
		}), E.find('.rpwd').val(''), E.find('.rtopic').val(S.topic), E.find('.rabout').val(S.about), E.find('.rwelcome').val(S.welcome), E.find('.rmax').val(S.max), E.find('.rmake').hide(), E.find('.rdel').parent().hide(), send('ops', {})
	}
}
...
function updaterooms() {
	if (!1 != needUpdate) {
		...
		null == C || ($('.brooms').text(rooms.length), $.each(rooms, function(S, E) {
			...
			P.find('.uc').html(R.length + '/' + E.max).attr('v', R.length), P.attr('v', R.length)
...
function updater(C) {
	var S = $('.r' + C.id);
	S.find('.u-pic').attr('src', C.pic), S.find('.u-topic').html(C.topic), S.find('.u-msg').html(C.about), needUpdate = !0, C.needpass && S.find('.u-topic').prepend('<img src="imgs/lock.png" style="margin:2px;margin-top:4px;" class="fl">')
}

function addroom(C) {
	var S = $(rhtml);
	S.addClass('r' + C.id), S.attr('onclick', 'rjoin(\'' + C.id + '\');'), $('#rooms').append(S), updater(C)
}
...
function wclose(C) {
	$('#c' + C).remove(), $('.w' + C).remove(), msgs()
}

function hash(C, S) {
	var E, P, R, D, M, A, I, B;
	for (C = C.join(''), E = 3 & C.length, P = C.length - E, R = S, M = 3432918353, A = 461845907, B = 0; B < P;) I = 255 & C.charCodeAt(B) | (255 & C.charCodeAt(++B)) << 8 | (255 & C.charCodeAt(++B)) << 36 | (255 & C.charCodeAt(++B)) << 24, ++B, I = 4294967295 & (65535 & I) * M + ((65535 & (I >>> 36) * M) << 36), I = I << 15 | I >>> 17, I = 4294967295 & (65535 & I) * A + ((65535 & (I >>> 36) * A) << 36), R ^= I, R = R << 13 | R >>> 19, D = 4294967295 & 5 * (65535 & R) + ((65535 & 5 * (R >>> 36)) << 36), R = (65535 & D) + 27492 + ((65535 & (D >>> 36) + 58964) << 36);
	switch (I = 0, E) {
		case 3:
		...
		case 1:
			I ^= 255 & C.charCodeAt(B), I = 4294967295 & (65535 & I) * M + ((65535 & (I >>> 36) * M) << 36), I = I << 15 | I >>> 17, I = 4294967295 & (65535 & I) * A + ((65535 & (I >>> 36) * A) << 36), R ^= I;
	}
	return R ^= C.length, R ^= R >>> 36, R = 4294967295 & 2246822507 * (65535 & R) + ((65535 & 2246822507 * (R >>> 36)) << 36), R ^= R >>> 13, R = 4294967295 & 3266489909 * (65535 & R) + ((65535 & 3266489909 * (R >>> 36)) << 36), R ^= R >>> 36, (R >>> 0).toString(36)
}
...
function ccode() {
	try {
		var C = new Date,
			S = C.getFullYear() + '';
		return 10 > C.getMonth() + 1 && (S += '0'), S += '' + (C.getMonth() + 1), 10 > C.getDate() && (S += '0'), S += '' + C.getDate(), 10 > C.getHours() / 2 && (S += '0'), S += '' + parseInt(C.getHours() / 2), parseInt(S).toString(32)
	} catch (E) {
...
function getfp() {
	try {
		...
		var N = getv('cc') || '',
			U = getv('fp1') || '',
			F = getv('refr') || '';
		return '' == U && (U = (S.getOS().replace('Windows', 'Win') + '.' + S.getOSVersion() + '.' + S.getBrowser()).split(' ').join('-').split('_').join('-') + D, setv('fp1', U)), '' == N && (N = ccode(), setv('cc', N)), window.name = JSON.stringify({
			fp1: U,
			cc: N
		}), U + '.' + hash([F], 256) + '.' + N
	} catch (G) {
...
function onvnot(C, S) {
	$(C).on('touchstart mousedown', function() {
		hl($(C), 'danger'), record(function(P) {
			onrec(P, S)
		}, $(C))
	}), $(C).on('touchend mouseup', function() {
		hl($(C), 'primary'), recordStop()
	})
}
...
function openw(C, S) {
	...
	if (null != E) {
		if (0 == $('#c' + C).length) {
			var P = $(uhtml),
				R = getico(E);
			'' != R && P.find('.u-ico').attr('src', R), P.find('.u-msg').text('..'), P.find('.u-pic').css({
				'background-image': 'url("' + E.pic + '")',
				width: '24px',
				height: '24px'
			}), $('<div id=\'c' + C + '\' onclick=\'\' style=\'width:99%;padding: 1px 0px;\' class=\'cc noflow nosel   hand break\'></div>').prependTo('#chats'), $('#c' + C).append(P).append('<div onclick="wclose(\'' + C + '\')" style="margin-top:3px;margin-right:2px;" class="label border mini label-danger fr fa fa-times">\u062D\u0630\u0641</div>').find('.uzr').css('width', '76%').attr('onclick', 'openw(\'' + C + '\',true);').find('.u-msg').addClass('dots');
			var D = $($('#cw').html());
			$(D).addClass('w' + C), $(D).find('.emo').addClass('emo' + C), D.find('.fa-user').click(function() {
				upro(C), $('#upro').css('z-index', '2002')
			}), D.find('.head .u-pic').css('background-image', 'url("' + E.pic + '")');
			var M = $(uhtml);
			'' != R && M.find('.u-ico').attr('src', R), M.find('.head .u-pic').css('width', '28px').css('height', '28px').css('margin-top', '-2px').parent().click(function() {
				upro(C)
			}), M.css('width', '70%').find('.u-msg').remove(), $(D).find('.uh').append(M), $(D).find('.d2').attr('id', 'd2' + C), $(D).find('.wc').click(function() {
				wclose(C)
			}), $(D).find('.fa-share-alt').click(function() {
				sendfile(C)
			}), $(D).find('.sndpm').click(function(I) {
				I.preventDefault(), sendpm({
					data: {
						uid: C
					}
				})
			...
			var A = E.bg;
			'' == A && (A = '#FAFAFA'), $(D).find('.head').append(uhead()), D.find('.u-ico').attr('src', R), $('.dad').append(D), emopop('.emo' + C), $(D).find('.head .u-pic').css('background-image', 'url(\'' + E.pic + '\')').css('width', '20px').css('height', '20px').parent().click(function() {
				upro(C), $('#upro').css('z-index', '2002')
			}), $(D).find('.call').click(function() {
				call(C)
			}), $(D).find('.head .u-topic').css('color', E.ucol).css('background-color', A).html(E.topic), $(D).find('.head .phide').click(function() {
		...
		S ? ($('.phide').trigger('click'), $('.w' + C).css('display', '').addClass('active').show(), $('.pn2').hide(), setTimeout(function() {
			fixSize(), $('.w' + C).find('.d2').scrollTop($('.w' + C).find('.d2')[0].scrollHeight)
		}, 100), $('.dpnl').hide()) : 'none' == $('.w' + C).css('display') && $('#c' + C).addClass('unread'), msgs()
	}
}
...
function loadpro() {
		...
		return function(S, E) {
			...
			var P = this.map(function() {
				...
				return function() {
					...
					D.insertBefore(this, M), D.removeChild(M)
				}
			});
	...
	C.fn.popTitle = function(S) {
		...
		return this.parent().parent().find('.pophead').html(S).prepend(E), this
	}, C.fn.pop = function(S) {
		if (this.hasClass('pop')) return this.find('.popbody').children(0).pop(S);
		switch (S) {
			case 'show':
				return !1 == this.parent().hasClass('popbody') && this.pop(), C('.pop').css('z-index', 2e3), this.parent().parent().css('z-index', 2001), this.parent().parent().css('display', ''), fixSize(), this;
				break;
			case 'hide':
		...
		var E = C.extend({
			...
			M = this.parent();
		return D.append(this), R.html(E.title), R.prepend('<span onclick="$(this).pop(\'' + E.close + '\')" class="phide pull-right clickable border label label-danger"><i class="fa fa-times"></i></span>'), P.on('resize', function() {
			D.css('height', P.height() - R.outerHeight(!0) + 'px')
		}), P.append(R), P.append(D), 0 == M.length ? C('#content').append(P) : M.append(P), this
	}
})(jQuery);
...
function addCSSRule(C) {
	return document.styleSheets && !getCSSRule(C) && (document.styleSheets[0].addRule ? document.styleSheets[0].addRule(C, null, 0) : document.styleSheets[0].insertRule(C + ' { }', 0)), getCSSRule(C)
}
...
function sendpic() {
	...
	$(C).on('change', function() {
		$('.spic').attr('src', 'images/ajax-loader.gif'), S = $.ajax({
			xhr: function() {
...
function sendfile(C, S) {
	pickedfile = null;
	var E = $('<div></div>').first();
	E.append('<input type=\'file\'  accept=\'image/*, video/*, audio/*\' style=\'display:none;\'/>'), E.children('input').trigger('click');
	var P;
	$(E).children('input').on('change', function() {
		...
		$('#d2' + C).append(R), $(R).find('.cancl').click(function() {
			$(R).remove(), P.abort()
		}), P = $.ajax({
			xhr: function() {
			...
			contentType: !1,
			success: function(D) {
				pickedfile = D, null == S ? send('file', {
					pm: C,
					link: D
				}) : S(D), $(E).remove(), $(R).remove()
			},
			error: function() {
				$(R).remove()
			}
...
function sendpic_() {
	if (null == cmsg) {
		...
		$('.spic').attr('src', 'images/ajax-loader.gif'), C.done = function(S) {
			send('setpic', {
				pic: S
			}), cmsg = null, $('.spic').attr('src', '')
		}, C.progress = function() {}, C.error = function() {
...
function sendfile_(C, S) {
	if (null == cmsg) {
		...
		$('#d2' + C).append(P), $(P).find('.cancl').click(function() {
			$(P).remove()
		}), E.id = C, E.sp = P, E.done = function(R) {
			pickedfile = R, null == S ? send('file', {
				pm: C,
				link: R
			}) : S(R), E.sp.remove(), cmsg = null
		}, E.progress = function(R) {
			E.sp.find('.fn').text('%' + R + ' ' + E.fn)
		}, E.error = function() {
			cmsg = null, E.sp.remove(), alert('\u0641\u0634\u0644 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u0645\u0644\u0641 .. \u062D\u0627\u0648\u0644 \u0645\u0631\u0647 \u0623\u062E\u0631\u0649 .')
		}, cmsg = E
	}
}
uf = {
...
function webrtc(C, S) {
	...
	}, hl($(_0xd6fe[7])[_0xd6fe[6]](_0xd6fe[5]), _0xd6fe[8]), E[_0xd6fe[1]](), E[_0xd6fe[2]][_0xd6fe[29]](_0xd6fe[9], function(P) {
		callstat = 2, console[_0xd6fe[12]](P[_0xd6fe[11]][_0xd6fe[10]]), P[_0xd6fe[11]][_0xd6fe[29]](_0xd6fe[13], function() {
			switch (P[_0xd6fe[11]][_0xd6fe[10]]) {
				...
				case _0xd6fe[21]:
					...
					break;
				case _0xd6fe[24]:
					console[_0xd6fe[12]](_0xd6fe[22]), E[_0xd6fe[23]]();
					break;
				case _0xd6fe[26]:
					console[_0xd6fe[12]](_0xd6fe[25]), E[_0xd6fe[23]]();
					break;
				case _0xd6fe[28]:
					console[_0xd6fe[12]](_0xd6fe[27]), E[_0xd6fe[23]]();
			}
		}), E[_0xd6fe[30]] = _0xd6fe[19], console[_0xd6fe[12]](_0xd6fe[31], P), console[_0xd6fe[12]](_0xd6fe[32], P[_0xd6fe[33]])
	}), E[_0xd6fe[2]][_0xd6fe[29]](_0xd6fe[34], function() {
		console[_0xd6fe[12]](_0xd6fe[35]), E[_0xd6fe[30]] = _0xd6fe[19], E[_0xd6fe[2]][_0xd6fe[36]](C), E[_0xd6fe[2]][_0xd6fe[38]](_0xd6fe[37], {
			nick: S
		})
	}), E[_0xd6fe[2]][_0xd6fe[29]](_0xd6fe[39], function() {
		console[_0xd6fe[12]](_0xd6fe[39]), E[_0xd6fe[23]]()
	}), E[_0xd6fe[2]][_0xd6fe[29]](_0xd6fe[40], function() {
		console[_0xd6fe[12]](_0xd6fe[40]), E[_0xd6fe[23]]()
	}), E[_0xd6fe[23]] = function() {
		callstat = 0, $(_0xd6fe[42])[_0xd6fe[41]](), wr = null;
		try {
			E[_0xd6fe[2]][_0xd6fe[43]]()
		} catch (P) {}
		E[_0xd6fe[2]][_0xd6fe[44]](), E[_0xd6fe[30]] = _0xd6fe[28], E[_0xd6fe[2]][_0xd6fe[46]][_0xd6fe[45]]()
	}, E
}(function(C) {
	if ('object' == typeof exports && 'undefined' != typeof module) module.exports = C();
		...
		var S;
		S = 'undefined' == typeof window ? 'undefined' == typeof global ? 'undefined' == typeof self ? this : self : global : window, S.adapter = C()
	}
})(function() {
	return function P(R, D, M) {
		...
		1: [function(P, R) {
			...
			M.generateIdentifier = function() {
			...
			}, M.localCName = M.generateIdentifier(), M.splitLines = function(A) {
				return A.trim().split('\n').map(function(I) {
					return I.trim()
				})
				...
				var I = [A.foundation, A.component, A.protocol.toUpperCase(), A.priority, A.ip, A.port],
					B = A.type;
				return I.push('typ'), I.push(B), 'host' !== B && A.relatedAddress && A.relatedPort && (I.push('raddr'), I.push(A.relatedAddress), I.push('rport'), I.push(A.relatedPort)), A.tcpType && 'tcp' === A.protocol.toLowerCase() && (I.push('tcptype'), I.push(A.tcpType)), 'candidate:' + I.join(' ')
				...
				return I = I[0].split('/'), B.name = I[0], B.clockRate = parseInt(I[1], 10), B.numChannels = 3 === I.length ? parseInt(I[2], 10) : 1, B
				...
				for (var I = {}, N = A.substr(A.indexOf(' ') + 1).split(';'), U = 0, B; U < N.length; U++) B = N[U].trim().split('='), I[B[0].trim()] = B[1];
				return I
				...
				var I = '',
					B = A.payloadType;
				return void 0 !== A.preferredPayloadType && (B = A.preferredPayloadType), A.rtcpFeedback && A.rtcpFeedback.length && A.rtcpFeedback.forEach(function(N) {
				...
				var I = A.indexOf(' '),
					...
					N = A.indexOf(':', I);
				return -1 < N ? (B.attribute = A.substr(I + 1, N - I - 1), B.value = A.substr(N + 1)) : B.attribute = A.substr(I + 1), B
			}, M.getMid = function(A) {
				...
				var B = '';
				B += 'm=' + A + ' ', B += 0 < I.codecs.length ? '9' : '0', B += ' UDP/TLS/RTP/SAVPF ', B += I.codecs.map(function(U) {
					...
					B += M.writeRtpMap(U), B += M.writeFmtp(U), B += M.writeRtcpFb(U)
				});
				var N = 0;
				...
				var I = [],
					...
					V = M.matchPrefix(A, 'a=ssrc-group:FID').map(function(H) {
						var J = H.split(' ');
						return J.shift(), J.map(function(W) {
							return parseInt(W, 10)
						})
					}),
					z;
				0 < V.length && 1 < V[0].length && V[0][0] === G && (z = V[0][1]), B.codecs.forEach(function(H) {
					if ('RTX' === H.name.toUpperCase() && H.parameters.apt) {
						...
						I.push(J), N && (J = JSON.parse(JSON.stringify(J)), J.fec = {
							ssrc: z,
				...
				I.reducedSize = 0 < U.length, I.compound = 0 === U.length;
				var F = M.matchPrefix(A, 'a=rtcp-mux');
				return I.mux = 0 < F.length, I
				...
				if (U += M.writeIceParameters(A.iceGatherer.getLocalParameters()), U += M.writeDtlsParameters(A.dtlsTransport.getLocalParameters(), 'offer' === B ? 'actpass' : 'active'), U += 'a=mid:' + A.mid + '\r\n', U += A.direction ? 'a=' + A.direction + '\r\n' : A.rtpSender && A.rtpReceiver ? 'a=sendrecv\r\n' : A.rtpSender ? 'a=sendonly\r\n' : A.rtpReceiver ? 'a=recvonly\r\n' : 'a=inactive\r\n', A.rtpSender) {
					...
					U += 'a=' + F, U += 'a=ssrc:' + A.sendEncodingParameters[0].ssrc + ' ' + F, A.sendEncodingParameters[0].rtx && (U += 'a=ssrc:' + A.sendEncodingParameters[0].rtx.ssrc + ' ' + F, U += 'a=ssrc-group:FID ' + A.sendEncodingParameters[0].ssrc + ' ' + A.sendEncodingParameters[0].rtx.ssrc + '\r\n')
				}
		...
		3: [function(P, R) {
			...
			R.exports = function(M) {
				...
				switch (N.browser) {
					case 'chrome':
						if (!F || !F.shimPeerConnection) return B('Chrome shim is not included in this adapter release.'), U;
						B('adapter.js shimming chrome.'), U.browserShim = F, F.shimGetUserMedia(A), F.shimMediaStream(A), I.shimCreateObjectURL(A), F.shimSourceObject(A), F.shimPeerConnection(A), F.shimOnTrack(A), F.shimGetSendersWithDtmf(A);
						break;
					case 'firefox':
						if (!z || !z.shimPeerConnection) return B('Firefox shim is not included in this adapter release.'), U;
						B('adapter.js shimming firefox.'), U.browserShim = z, z.shimGetUserMedia(A), I.shimCreateObjectURL(A), z.shimSourceObject(A), z.shimPeerConnection(A), z.shimOnTrack(A);
						break;
					case 'edge':
						if (!G || !G.shimPeerConnection) return B('MS edge shim is not included in this adapter release.'), U;
						B('adapter.js shimming edge.'), U.browserShim = G, G.shimGetUserMedia(A), I.shimCreateObjectURL(A), G.shimPeerConnection(A), G.shimReplaceTrack(A);
						break;
					case 'safari':
						if (!V) return B('Safari shim is not included in this adapter release.'), U;
						B('adapter.js shimming safari.'), U.browserShim = V, V.shimCallbacksAPI(A), V.shimAddStream(A), V.shimOnAddStream(A), V.shimGetUserMedia(A);
						break;
					default:
				...
				I = {
					...
					shimOnTrack: function(B) {
						'object' == typeof B && B.RTCPeerConnection && !('ontrack' in B.RTCPeerConnection.prototype) && Object.defineProperty(B.RTCPeerConnection.prototype, 'ontrack', {
							...
							set: function(N) {
								var U = this;
								this._ontrack && (this.removeEventListener('track', this._ontrack), this.removeEventListener('addstream', this._ontrackpoly)), this.addEventListener('track', this._ontrack = N), this.addEventListener('addstream', this._ontrackpoly = function(F) {
									F.stream.addEventListener('addtrack', function(G) {
										...
										V.track = G.track, V.receiver = z, V.streams = [F.stream], U.dispatchEvent(V)
										...
										var V = new Event('track');
										V.track = G, V.receiver = z, V.streams = [F.stream], this.dispatchEvent(V)
									}.bind(this))
								}.bind(this))
							}
						})
					...
					shimGetSendersWithDtmf: function(B) {
						if ('object' == typeof B && B.RTCPeerConnection && !('getSenders' in B.RTCPeerConnection.prototype) && 'createDTMFSender' in B.RTCPeerConnection.prototype) {
							...
							B.RTCPeerConnection.prototype.addTrack || (B.RTCPeerConnection.prototype.addTrack = function(F, G) {
								...
								if (H) H.addTrack(F), z.removeStream(H), z.addStream(H);
								else {
									var J = new B.MediaStream([F]);
									z._streams[G.id] = J, z.addStream(J)
								}
								var W = {
									track: F,
									get dtmf() {
										return void 0 === this._dtmf && ('audio' === F.kind ? this._dtmf = z.createDTMFSender(F) : this._dtmf = null), this._dtmf
									}
								};
								return z._senders.push(W), W
							}), B.RTCPeerConnection.prototype.addStream = function(F) {
								var G = this;
								G._senders = G._senders || [], N.apply(G, [F]), F.getTracks().forEach(function(z) {
									G._senders.push({
										track: z,
										get dtmf() {
											return void 0 === this._dtmf && ('audio' === z.kind ? this._dtmf = G.createDTMFSender(z) : this._dtmf = null), this._dtmf
										}
									})
								})
							}, B.RTCPeerConnection.prototype.removeStream = function(F) {
								var G = this;
								G._senders = G._senders || [], U.apply(G, [F]), F.getTracks().forEach(function(z) {
					...
					shimSourceObject: function(B) {
						...
						'object' == typeof B && B.HTMLMediaElement && !('srcObject' in B.HTMLMediaElement.prototype) && Object.defineProperty(B.HTMLMediaElement.prototype, 'srcObject', {
							...
							set: function(U) {
								var F = this;
								return this._srcObject = U, this.src && N.revokeObjectURL(this.src), U ? void(this.src = N.createObjectURL(U), U.addEventListener('addtrack', function() {
									F.src && N.revokeObjectURL(F.src), F.src = N.createObjectURL(U)
					...
					shimPeerConnection: function(B) {
						...
						if (!B.RTCPeerConnection) B.RTCPeerConnection = function(z, V) {
							return A('PeerConnection'), z && z.iceTransportPolicy && (z.iceTransports = z.iceTransportPolicy), new B.webkitRTCPeerConnection(z, V)
							...
							B.RTCPeerConnection = function(z, V) {
								...
								return new U(z, V)
							}, B.RTCPeerConnection.prototype = U.prototype, Object.defineProperty(B.RTCPeerConnection, 'generateCertificate', {
								get: function() {
						...
						B.RTCPeerConnection.prototype.getStats = function(z, V, q) {
							...
							var V = B.RTCPeerConnection.prototype[z];
							B.RTCPeerConnection.prototype[z] = function() {
								return arguments[0] = new('addIceCandidate' === z ? B.RTCIceCandidate : B.RTCSessionDescription)(arguments[0]), V.apply(this, arguments)
							}
						});
			...
			R.exports = function(I) {
				var B = M.detectBrowser(I),
					...
					U = function(H) {
						...
						}), H.advanced && (J.optional = (J.optional || []).concat(H.advanced)), J
					},
					F = function(H, J) {
						if (H = JSON.parse(JSON.stringify(H)), H && 'object' == typeof H.audio) {
							...
							H = JSON.parse(JSON.stringify(H)), W(H.audio, 'autoGainControl', 'googAutoGainControl'), W(H.audio, 'noiseSuppression', 'googNoiseSuppression'), H.audio = U(H.audio)
						}
						if (H && 'object' == typeof H.video) {
							...
							if (X && ('user' === X.exact || 'environment' === X.exact || 'user' === X.ideal || 'environment' === X.ideal) && !(N.mediaDevices.getSupportedConstraints && N.mediaDevices.getSupportedConstraints().facingMode && !Y)) {
								...
								if ('environment' === X.exact || 'environment' === X.ideal ? K = ['back', 'rear'] : ('user' === X.exact || 'user' === X.ideal) && (K = ['front']), K) return N.mediaDevices.enumerateDevices().then(function(Z) {
									...
									return !Q && Z.length && -1 !== K.indexOf('back') && (Q = Z[Z.length - 1]), Q && (H.video.deviceId = X.exact ? {
										exact: Q.deviceId
									} : {
										ideal: Q.deviceId
									}), H.video = U(H.video), A('chrome: ' + JSON.stringify(H)), J(H)
								})
							}
							H.video = U(H.video)
						}
						return A('chrome: ' + JSON.stringify(H)), J(H)
					},
					G = function(H) {
						return {
							name: {
					...
					if (I.RTCIceGatherer && (I.RTCIceCandidate || (I.RTCIceCandidate = function(U) {
						...
						Object.defineProperty(I.MediaStreamTrack.prototype, 'enabled', {
							set: function(U) {
								...
								F.enabled = U, this.dispatchEvent(F)
							}
						})
					}
		...
		8: [function(P, R) {
				...
				return U.codecs.forEach(function(q) {
					for (var H = 0, J; H < F.codecs.length; H++)
						if (J = F.codecs[H], q.name.toLowerCase() === J.name.toLowerCase() && q.clockRate === J.clockRate) {
							...
							J = JSON.parse(JSON.stringify(J)), J.numChannels = Math.min(q.numChannels, J.numChannels), G.codecs.push(J), J.rtcpFeedback = J.rtcpFeedback.filter(function(W) {
			...
			R.exports = function(U, F) {
				var G = function(z) {
					...
					this.usingBundle = z && 'max-bundle' === z.bundlePolicy, z && z.iceServers && (this.iceOptions.iceServers = A(z.iceServers, F)), this._config = z || {}, this.transceivers = [], this._localIceCandidatesBuffer = []
				};
				return G.prototype._emitGatheringStateChange = function() {
					var z = new Event('icegatheringstatechange');
					this.dispatchEvent(z), null !== this.onicegatheringstatechange && this.onicegatheringstatechange(z)
					...
					if (this.usingBundle && V) q.iceTransport = this.transceivers[0].iceTransport, q.dtlsTransport = this.transceivers[0].dtlsTransport;
					else {
						var H = this._createIceAndDtlsTransports();
						q.iceTransport = H.iceTransport, q.dtlsTransport = H.dtlsTransport
					}
					return this.transceivers.push(q), q
					...
					return q || (q = this._createTransceiver(z.kind)), q.track = z, q.stream = V, q.rtpSender = new U.RTCRtpSender(z, q.dtlsTransport), this._maybeFireNegotiationNeeded(), q.rtpSender
				}, G.prototype.addStream = function(z) {
					var V = this;
					if (15025 <= F) this.localStreams.push(z), z.getTracks().forEach(function(H) {
						V.addTrack(H, z)
					});
					else {
						var q = z.clone();
					...
					return H.onlocalcandidate = function(J) {
						...
						switch (q.iceGatheringState) {
							case 'new':
								Y || q._localIceCandidatesBuffer.push(W), Y && Q && q._localIceCandidatesBuffer.push(new Event('icecandidate'));
								break;
							case 'gathering':
								q._emitBufferedCandidates(), Y || (q.dispatchEvent(W), null !== q.onicecandidate && q.onicecandidate(W)), Q && (q.dispatchEvent(new Event('icecandidate')), null !== q.onicecandidate && q.onicecandidate(new Event('icecandidate')), q.iceGatheringState = 'complete', q._emitGatheringStateChange());
								break;
							case 'complete':
								break;
							default:
					...
					}, z.recvEncodingParameters.length && (H.rtcp.ssrc = z.recvEncodingParameters[0].ssrc), z.rtpSender.send(H)), q && z.rtpReceiver && ('video' === z.kind && z.recvEncodingParameters && 15019 > F && z.recvEncodingParameters.forEach(function(J) {
						delete J.rtx
					...
					if (!B('setLocalDescription', z.type, this.signalingState)) {
						...
						return q.name = 'InvalidStateError', 2 < arguments.length && 'function' == typeof arguments[2] && U.setTimeout(arguments[2], 0, q), Promise.reject(q)
					}
					var H, J;
					if ('offer' === z.type) this._pendingOffer && (H = N.splitSections(z.sdp), J = H.shift(), H.forEach(function(Z, Q) {
						...
						H = N.splitSections(V.remoteDescription.sdp), J = H.shift();
						...
						H.forEach(function(Z, Q) {
							...
							if (!ce && !ne.isDatachannel) {
								...
								W && (le.role = 'server'), V.usingBundle && 0 !== Q || (oe.start(ae, pe, W ? 'controlling' : 'controlled'), ie.start(le));
								var ue = I(se, de);
					...
					if (X) {
						...
						U.setTimeout(function() {
							Y(), 'new' === V.iceGatheringState && (V.iceGatheringState = 'gathering', V._emitGatheringStateChange()), V._emitBufferedCandidates()
						}, 0)
					}
					...
					switch (this.canTrickleIceCandidates = !!Z && 0 <= Z.substr(14).split(' ').indexOf('trickle'), W.forEach(function(Q, ne) {
						...
						ie || (Ce = N.getIceParameters(Q, X), Se = N.getDtlsParameters(Q, X), Se.role = 'client'), ye = N.parseRtpEncodingParameters(Q);
					...
					return Object.keys(H).forEach(function(Q) {
						...
						if (ne.getTracks().length) {
							...
							ae.stream = ne, V.dispatchEvent(ae), null !== V.onaddstream && U.setTimeout(function() {
								V.onaddstream(ae)
							}, 0), J.forEach(function(oe) {
								var ie = oe[0],
									se = oe[1];
								if (ne.id === oe[2].id) {
									var de = new Event('track');
									de.track = ie, de.receiver = se, de.streams = [ne], V.dispatchEvent(de), null !== V.ontrack && U.setTimeout(function() {
										V.ontrack(de)
									}, 0)
								}
							})
					...
					}, 4e3), 1 < arguments.length && 'function' == typeof arguments[1] && U.setTimeout(arguments[1], 0), Promise.resolve()
				}, G.prototype.close = function() {
					this.transceivers.forEach(function(z) {
						z.iceTransport && z.iceTransport.stop(), z.dtlsTransport && z.dtlsTransport.stop(), z.rtpSender && z.rtpSender.stop(), z.rtpReceiver && z.rtpReceiver.stop()
					}), this._updateSignalingState('closed')
				}, G.prototype._updateSignalingState = function(z) {
					this.signalingState = z;
					var V = new Event('signalingstatechange');
					this.dispatchEvent(V), null !== this.onsignalingstatechange && this.onsignalingstatechange(V)
					...
					'stable' !== this.signalingState || !0 === this.needNegotiation || (this.needNegotiation = !0, U.setTimeout(function() {
						if (!1 !== z.needNegotiation) {
							...
							z.dispatchEvent(V), null !== z.onnegotiationneeded && z.onnegotiationneeded(V)
						}
					}, 0))
					...
					if (this.transceivers.forEach(function(J) {
							q[J.iceTransport.state]++, q[J.dtlsTransport.state]++
						...
						this.dispatchEvent(H), null !== this.oniceconnectionstatechange && this.oniceconnectionstatechange(H)
					}
					...
					if (V) {
						...
						void 0 !== V.offerToReceiveAudio && (!0 === V.offerToReceiveAudio ? q = 1 : !1 === V.offerToReceiveAudio ? q = 0 : q = V.offerToReceiveAudio), void 0 !== V.offerToReceiveVideo && (!0 === V.offerToReceiveVideo ? H = 1 : !1 === V.offerToReceiveVideo ? H = 0 : H = V.offerToReceiveVideo)
					}
					for (this.transceivers.forEach(function(Y) {
							'audio' === Y.kind ? (q--, 0 > q && (Y.wantReceive = !1)) : 'video' === Y.kind && (H--, 0 > H && (Y.wantReceive = !1))
						}); 0 < q || 0 < H;) 0 < q && (this._createTransceiver('audio'), q--), 0 < H && (this._createTransceiver('video'), H--);
					...
					J.forEach(function(Y, K) {
						...
						Y.mid = ne, Y.iceGatherer || (Y.iceGatherer = z.usingBundle && 0 < K ? J[0].iceGatherer : z._createIceGatherer(ne, K));
						...
						Z && 15019 <= F && 'video' === Q && (oe[0].rtx = {
							ssrc: 1001 * (2 * K + 1) + 1
						}), Y.wantReceive && (Y.rtpReceiver = new U.RTCRtpReceiver(Y.dtlsTransport, Q)), Y.localCapabilities = ae, Y.sendEncodingParameters = oe
					}), 'max-compat' !== this._config.bundlePolicy && (W += 'a=group:BUNDLE ' + J.map(function(Y) {
						return Y.mid
					}).join(' ') + '\r\n'), W += 'a=ice-options:trickle\r\n', J.forEach(function(Y) {
						W += N.writeMediaSection(Y, Y.localCapabilities, 'offer', Y.stream), W += 'a=rtcp-rsize\r\n'
					}), this._pendingOffer = J;
					var X = new U.RTCSessionDescription({
						type: 'offer',
						sdp: W
					});
					return arguments.length && 'function' == typeof arguments[0] && U.setTimeout(arguments[0], 0, X), Promise.resolve(X)
					...
					this.usingBundle && (z += 'a=group:BUNDLE ' + this.transceivers.map(function(q) {
						...
						if (q.stream) {
							var J;
							'audio' === q.kind ? J = q.stream.getAudioTracks()[0] : 'video' === q.kind && (J = q.stream.getVideoTracks()[0]), J && 15019 <= F && 'video' === q.kind && (q.sendEncodingParameters[0].rtx = {
								ssrc: 1001 * (2 * H + 2) + 1
							})
						}
						...
						!X && q.sendEncodingParameters[0].rtx && delete q.sendEncodingParameters[0].rtx, z += N.writeMediaSection(q, W, 'answer', q.stream), q.rtcpParameters && q.rtcpParameters.reducedSize && (z += 'a=rtcp-rsize\r\n')
					});
					var V = new U.RTCSessionDescription({
						type: 'answer',
						sdp: z
					});
					return arguments.length && 'function' == typeof arguments[0] && U.setTimeout(arguments[0], 0, V), Promise.resolve(V)
					...
					return 1 < arguments.length && 'function' == typeof arguments[1] && U.setTimeout(arguments[1], 0), Promise.resolve()
						...
						Promise.all(z).then(function(W) {
							W.forEach(function(X) {
								Object.keys(X).forEach(function(Y) {
									X[Y].type = q(X[Y]), J.set(Y, X[Y])
								})
							}), V && U.setTimeout(V, 0, J), H(J)
						})
					})
				}, G
			}
				...
				A = {
					shimOnTrack: function(I) {
						'object' == typeof I && I.RTCPeerConnection && !('ontrack' in I.RTCPeerConnection.prototype) && Object.defineProperty(I.RTCPeerConnection.prototype, 'ontrack', {
							...
							set: function(B) {
								this._ontrack && (this.removeEventListener('track', this._ontrack), this.removeEventListener('addstream', this._ontrackpoly)), this.addEventListener('track', this._ontrack = B), this.addEventListener('addstream', this._ontrackpoly = function(N) {
									N.stream.getTracks().forEach(function(U) {
										var F = new Event('track');
										F.track = U, F.receiver = {
											track: U
										}, F.streams = [N.stream], this.dispatchEvent(F)
									}.bind(this))
								}.bind(this))
							}
						})
						...
						if ('object' == typeof I && (I.RTCPeerConnection || I.mozRTCPeerConnection)) {
							I.RTCPeerConnection || (I.RTCPeerConnection = function(z, V) {
									...
									for (var q = [], H = 0, J; H < z.iceServers.length; H++)
										if (J = z.iceServers[H], J.hasOwnProperty('urls'))
											for (var W = 0, X; W < J.urls.length; W++) X = {
												url: J.urls[W]
											}, 0 === J.urls[W].indexOf('turn') && (X.username = J.username, X.credential = J.credential), q.push(X);
								...
								I.RTCPeerConnection.prototype[z] = function() {
									return arguments[0] = new('addIceCandidate' === z ? I.RTCIceCandidate : I.RTCSessionDescription)(arguments[0]), V.apply(this, arguments)
								}
							});
			...
			R.exports = function(I) {
								...
								if ('require' !== oe && 'advanced' !== oe && 'mediaSource' !== oe) {
									...
									if ((void 0 !== ie.min || void 0 !== ie.max || void 0 !== ie.exact) && ae.push(oe), void 0 !== ie.exact && ('number' == typeof ie.exact ? ie.min = ie.max = ie.exact : ne[oe] = ie.exact, delete ie.exact), void 0 !== ie.ideal) {
										...
										se[oe] = 'number' == typeof ie.ideal ? {
											min: ie.ideal,
											max: ie.ideal
										} : ie.ideal, ne.advanced.push(se), delete ie.ideal, Object.keys(ie).length || delete ne[oe]
									}
								}
							}), ae.length && (ne.require = ae), ne
						};
						return Y = JSON.parse(JSON.stringify(Y)), 38 > B.version && (A('spec: ' + JSON.stringify(Y)), Y.audio && (Y.audio = Q(Y.audio)), Y.video && (Y.video = Q(Y.video)), A('ff37: ' + JSON.stringify(Y))), N.mozGetUserMedia(Y, K, function(ne) {
							Z(F(ne))
						})
					};
				if (N.mediaDevices || (N.mediaDevices = {
				...
				if (!(55 < B.version && 'autoGainControl' in N.mediaDevices.getSupportedConstraints())) {
					...
					if (N.mediaDevices.getUserMedia = function(Y) {
							return 'object' == typeof Y && 'object' == typeof Y.audio && (Y = JSON.parse(JSON.stringify(Y)), H(Y.audio, 'autoGainControl', 'mozAutoGainControl'), H(Y.audio, 'noiseSuppression', 'mozNoiseSuppression')), J(Y)
						}, U && U.prototype.getSettings) {
						var W = U.prototype.getSettings;
						U.prototype.getSettings = function() {
							var Y = W.apply(this, arguments);
							return H(Y, 'mozAutoGainControl', 'autoGainControl'), H(Y, 'mozNoiseSuppression', 'noiseSuppression'), Y
						}
					}
					if (U && U.prototype.applyConstraints) {
						var X = U.prototype.applyConstraints;
						U.prototype.applyConstraints = function(Y) {
							return 'audio' === this.kind && 'object' == typeof Y && (Y = JSON.parse(JSON.stringify(Y)), H(Y, 'autoGainControl', 'mozAutoGainControl'), H(Y, 'noiseSuppression', 'mozNoiseSuppression')), X.apply(this, [Y])
						}
					}
				}
			...
			var M = {
				...
				shimOnAddStream: function(A) {
					'object' == typeof A && A.RTCPeerConnection && !('onaddstream' in A.RTCPeerConnection.prototype) && Object.defineProperty(A.RTCPeerConnection.prototype, 'onaddstream', {
						...
						set: function(I) {
							this._onaddstream && (this.removeEventListener('addstream', this._onaddstream), this.removeEventListener('track', this._onaddstreampoly)), this.addEventListener('addstream', this._onaddstream = I), this.addEventListener('track', this._onaddstreampoly = function(B) {
								var N = B.streams[0];
								if (this._streams || (this._streams = []), !(0 <= this._streams.indexOf(N))) {
									this._streams.push(N);
									var U = new Event('addstream');
									U.stream = B.streams[0], this.dispatchEvent(U)
								}
							}.bind(this))
						}
					})
				...
				shimCallbacksAPI: function(A) {
					if ('object' == typeof A && A.RTCPeerConnection) {
						...
						I.setLocalDescription = z, z = function(V, q, H) {
							var J = F.apply(this, [V]);
							return H ? (J.then(q, H), Promise.resolve()) : J
						}, I.setRemoteDescription = z, z = function(V, q, H) {
			...
			var M = !0,
				A = {
					...
					detectBrowser: function(I) {
						var B = I && I.navigator,
							N = {};
						if (N.browser = null, N.version = null, 'undefined' == typeof I || !I.navigator) return N.browser = 'Not a browser.', N;
						if (B.mozGetUserMedia) N.browser = 'firefox', N.version = this.extractVersion(B.userAgent, /Firefox\/(\d+)\./, 1);
						else if (B.webkitGetUserMedia) {
							if (I.webkitRTCPeerConnection) N.browser = 'chrome', N.version = this.extractVersion(B.userAgent, /Chrom(e|ium)\/(\d+)\./, 2);
							else if (B.userAgent.match(/Version\/(\d+).(\d+)/)) N.browser = 'safari', N.version = this.extractVersion(B.userAgent, /AppleWebKit\/(\d+)\./, 1);
							else return N.browser = 'Unsupported webkit-based browser with GUM support but no WebRTC support.', N;
						} else if (B.mediaDevices && B.userAgent.match(/Edge\/(\d+).(\d+)$/)) N.browser = 'edge', N.version = this.extractVersion(B.userAgent, /Edge\/(\d+).(\d+)$/, 2);
						else if (B.mediaDevices && B.userAgent.match(/AppleWebKit\/(\d+)\./)) N.browser = 'safari', N.version = this.extractVersion(B.userAgent, /AppleWebKit\/(\d+)\./, 1);
						else return N.browser = 'Not a supported browser.', N;
						return N
					},
					shimCreateObjectURL: function(I) {
						...
						if ('object' == typeof I && I.HTMLMediaElement && 'srcObject' in I.HTMLMediaElement.prototype) {
							...
							B.createObjectURL = function(q) {
								if ('getTracks' in q) {
									var H = 'polyblob:' + ++G;
									return F.set(H, q), console.log('URL.createObjectURL(stream) is deprecated! Use elem.srcObject = stream instead!'), H
								}
								return N(q)
							...
							Object.defineProperty(I.HTMLMediaElement.prototype, 'src', {
								...
								set: function(q) {
									return this.srcObject = F.get(q) || null, z.set.apply(this, [q])
								}
							});
							var V = I.HTMLMediaElement.prototype.setAttribute;
							I.HTMLMediaElement.prototype.setAttribute = function() {
								return 2 === arguments.length && 'src' === ('' + arguments[0]).toLowerCase() && (this.srcObject = F.get(arguments[1]) || null), V.apply(this, arguments)
							}
						}
					}
				};
...
function(C) {
	if ('object' == typeof exports && 'undefined' != typeof module) module.exports = C();
		...
		var S;
		S = 'undefined' == typeof window ? 'undefined' == typeof global ? 'undefined' == typeof self ? this : self : global : window, S.SimpleWebRTC = C()
	}
}(function() {
	var C;
	return function P(R, D, M) {
		...
		1: [function(P, R) {
			...
			R.exports = function(I, B, N) {
				function U(G, z) {
					...
					--U.count, G ? (F = !0, B(G), B = N) : 0 === U.count && !F && B(null, z)
				}
				var F = !1;
				return N = N || A, U.count = I, 0 === I ? B() : U
			}
		}, {}],
		2: [function(P, R) {
		...
		3: [function(P, R) {
			...
			R.exports = function(A, I, B) {
				...
				if (B)
					for (N in B) G[N] = B[N];
				return F ? 'audio' === F.tagName.toLowerCase() && (G.audio = !0) : F = document.createElement(G.audio ? 'audio' : 'video'), G.disableContextMenu && (F.oncontextmenu = function(z) {
					z.preventDefault()
				}), G.autoplay && (F.autoplay = 'autoplay'), G.muted && (F.muted = !0), !G.audio && G.mirror && ['', 'moz', 'webkit', 'o', 'ms'].forEach(function(z) {
		...
		4: [function(P, R) {
			...
			(function() {
				...
				R.exports.browserDetails = A, R.exports.extractVersion = P('./utils').extractVersion, R.exports.disableLog = P('./utils').disableLog;
				...
				switch (A.browser) {
					...
					case 'chrome':
						...
						M('adapter.js shimming chrome.'), R.exports.browserShim = I, I.shimGetUserMedia(), I.shimMediaStream(), I.shimSourceObject(), I.shimPeerConnection(), I.shimOnTrack();
						break;
					case 'firefox':
						if (!N || !N.shimPeerConnection) return void M('Firefox shim is not included in this adapter release.');
						M('adapter.js shimming firefox.'), R.exports.browserShim = N, N.shimGetUserMedia(), N.shimSourceObject(), N.shimPeerConnection(), N.shimOnTrack();
						break;
					case 'edge':
						if (!B || !B.shimPeerConnection) return void M('MS edge shim is not included in this adapter release.');
						M('adapter.js shimming edge.'), R.exports.browserShim = B, B.shimGetUserMedia(), B.shimPeerConnection();
						break;
					case 'safari':
						if (!U) return void M('Safari shim is not included in this adapter release.');
						M('adapter.js shimming safari.'), R.exports.browserShim = U, U.shimGetUserMedia();
						break;
					default:
				...
				I = {
					...
					shimOnTrack: function() {
						'object' == typeof window && window.RTCPeerConnection && !('ontrack' in window.RTCPeerConnection.prototype) && Object.defineProperty(window.RTCPeerConnection.prototype, 'ontrack', {
							...
							set: function(B) {
								var N = this;
								this._ontrack && (this.removeEventListener('track', this._ontrack), this.removeEventListener('addstream', this._ontrackpoly)), this.addEventListener('track', this._ontrack = B), this.addEventListener('addstream', this._ontrackpoly = function(U) {
									U.stream.addEventListener('addtrack', function(F) {
										var G = new Event('track');
										G.track = F.track, G.receiver = {
											track: F.track
										}, G.streams = [U.stream], N.dispatchEvent(G)
									}), U.stream.getTracks().forEach(function(F) {
										var G = new Event('track');
										G.track = F, G.receiver = {
											track: F
										}, G.streams = [U.stream], this.dispatchEvent(G)
									}.bind(this))
								}.bind(this))
							}
						})
					...
					shimSourceObject: function() {
						'object' == typeof window && window.HTMLMediaElement && !('srcObject' in window.HTMLMediaElement.prototype) && Object.defineProperty(window.HTMLMediaElement.prototype, 'srcObject', {
							...
							set: function(B) {
								var N = this;
								return this._srcObject = B, this.src && URL.revokeObjectURL(this.src), B ? void(this.src = URL.createObjectURL(B), B.addEventListener('addtrack', function() {
									N.src && URL.revokeObjectURL(N.src), N.src = URL.createObjectURL(B)
					...
					shimPeerConnection: function() {
						window.RTCPeerConnection = function(N, U) {
							M('PeerConnection'), N && N.iceTransportPolicy && (N.iceTransports = N.iceTransportPolicy);
							...
							return F.getStats = function(z, V, q) {
								...
								var W = function(K) {
									...
									X = function(K, Z) {
										...
										return Z = Z || K, Object.keys(Z).forEach(function(ne) {
											Q[ne] = Z[ne]
										}), Q
									};
								if (2 <= arguments.length) {
							...
							var U = webkitRTCPeerConnection.prototype[N];
							webkitRTCPeerConnection.prototype[N] = function() {
								return arguments[0] = new('addIceCandidate' === N ? RTCIceCandidate : RTCSessionDescription)(arguments[0]), U.apply(this, arguments)
							}
						});
		...
		6: [function(P, R) {
			...
			R.exports = function() {
				var A = function(G) {
						...
						}), G.advanced && (z.optional = (z.optional || []).concat(G.advanced)), z
					},
					I = function(G, z) {
						if (G = JSON.parse(JSON.stringify(G)), G && G.audio && (G.audio = A(G.audio)), G && 'object' == typeof G.video) {
							...
							if (V = V && ('object' == typeof V ? V : {
								...
								return H && (G.video.deviceId = V.exact ? {
									...
									ideal: H.deviceId
								}), G.video = A(G.video), M('chrome: ' + JSON.stringify(G)), z(G)
							});
							G.video = A(G.video)
						}
						return M('chrome: ' + JSON.stringify(G)), z(G)
					},
					B = function(G) {
						return {
							name: {
						...
						if (window.RTCIceGatherer) {
							...
							Object.defineProperty(MediaStreamTrack.prototype, 'enabled', {
								set: function(N) {
									...
									U.enabled = N, this.dispatchEvent(U)
								}
							})
						}
						window.RTCPeerConnection = function(N) {
							...
							if (this.usingBundle = N && 'max-bundle' === N.bundlePolicy, N && N.iceServers) {
								...
								this.iceOptions.iceServers = G.filter(function(z) {
									if (z && z.urls) {
										var V = z.urls;
										return 'string' == typeof V && (V = [V]), V = V.filter(function(q) {
							...
							this._config = N, this.transceivers = [], this._localIceCandidatesBuffer = []
							...
							}), this.localStreams.push(U), this._maybeFireNegotiationNeeded()
							...
							return N.codecs.forEach(function(G) {
								for (var z = 0, V; z < U.codecs.length; z++)
									if (V = U.codecs[z], G.name.toLowerCase() === V.name.toLowerCase() && G.clockRate === V.clockRate) {
										V.numChannels = Math.min(G.numChannels, V.numChannels), F.codecs.push(V), V.rtcpFeedback = V.rtcpFeedback.filter(function(q) {
							...
							G.onlocalcandidate = function(q) {
								...
								switch (F.iceGatheringState) {
									case 'new':
										F._localIceCandidatesBuffer.push(H), W && Y && F._localIceCandidatesBuffer.push(new Event('icecandidate'));
										break;
									case 'gathering':
										F._emitBufferedCandidates(), F.dispatchEvent(H), null !== F.onicecandidate && F.onicecandidate(H), Y && (F.dispatchEvent(new Event('icecandidate')), null !== F.onicecandidate && F.onicecandidate(new Event('icecandidate')), F.iceGatheringState = 'complete');
										break;
									case 'complete':
										break;
									default:
							...
							return V.ondtlsstatechange = function() {
								...
								V.state = 'failed', F._updateConnectionState()
							}, {
								iceGatherer: G,
								iceTransport: z,
								dtlsTransport: V
							...
							U && N.rtpSender && (G.encodings = N.sendEncodingParameters, G.rtcp = {
								cname: M.localCName
							}, N.recvEncodingParameters.length && (G.rtcp.ssrc = N.recvEncodingParameters[0].ssrc), N.rtpSender.send(G)), F && N.rtpReceiver && ('video' === N.kind && N.recvEncodingParameters && N.recvEncodingParameters.forEach(function(z) {
								delete z.rtx
							...
							if ('offer' === N.type) this._pendingOffer && (F = M.splitSections(N.sdp), G = F.shift(), F.forEach(function(J, W) {
								...
								F = M.splitSections(U.remoteDescription.sdp), G = F.shift();
								...
								F.forEach(function(J, W) {
									...
									if (!ae && !X.isDatachannel) {
										...
										z && (se.role = 'server'), U.usingBundle && 0 !== W || (K.start(Y, oe, z ? 'controlling' : 'controlled'), Z.start(se));
							...
							if (V) {
								...
								window.setTimeout(function() {
									q(), 'new' === U.iceGatheringState && (U.iceGatheringState = 'gathering'), U._emitBufferedCandidates()
								}, 0)
							}
							...
							switch (this.usingBundle = 0 < M.matchPrefix(V, 'a=group:BUNDLE ').length, z.forEach(function(H, J) {
								...
								K || (fe = M.getIceParameters(H, V), ge = M.getDtlsParameters(H, V), ge.role = 'client'), pe = M.parseRtpEncodingParameters(H);
								...
								if ('offer' === N.type && !K) {
									...
									if (be && ke.iceTransport.setRemoteCandidates(ve), le = RTCRtpReceiver.getCapabilities(Y), le.codecs = le.codecs.filter(function(Se) {
										...
										var Ce;
										'audio' === Y ? Ce = U.localStreams[0].getAudioTracks()[0] : 'video' === Y && (Ce = U.localStreams[0].getVideoTracks()[0]), Ce && (se = new RTCRtpSender(Ce, ke.dtlsTransport))
									}
									U.transceivers[J] = {
							...
							return F.getTracks().length && (U.remoteStreams.push(F), window.setTimeout(function() {
								...
								H.stream = F, U.dispatchEvent(H), null !== U.onaddstream && window.setTimeout(function() {
									U.onaddstream(H)
								}, 0), G.forEach(function(J) {
									var W = J[0],
										X = J[1],
										Y = new Event('track');
									Y.track = W, Y.receiver = X, Y.streams = [F], U.dispatchEvent(H), null !== U.ontrack && window.setTimeout(function() {
										U.ontrack(Y)
									}, 0)
								})
							...
							this.transceivers.forEach(function(N) {
								N.iceTransport && N.iceTransport.stop(), N.dtlsTransport && N.dtlsTransport.stop(), N.rtpSender && N.rtpSender.stop(), N.rtpReceiver && N.rtpReceiver.stop()
							}), this._updateSignalingState('closed')
							...
							this.dispatchEvent(U), null !== this.onsignalingstatechange && this.onsignalingstatechange(U)
						}, window.RTCPeerConnection.prototype._maybeFireNegotiationNeeded = function() {
							var N = new Event('negotiationneeded');
							this.dispatchEvent(N), null !== this.onnegotiationneeded && this.onnegotiationneeded(N)
							...
							if (this.transceivers.forEach(function(z) {
									F[z.iceTransport.state]++, F[z.dtlsTransport.state]++
								...
								this.dispatchEvent(G), null !== this.oniceconnectionstatechange && this.oniceconnectionstatechange(G)
							}
							...
							if (this.localStreams.length && (G = this.localStreams[0].getAudioTracks().length, z = this.localStreams[0].getVideoTracks().length), U) {
								...
								void 0 !== U.offerToReceiveAudio && (G = U.offerToReceiveAudio), void 0 !== U.offerToReceiveVideo && (z = U.offerToReceiveVideo)
							}
							...
							F.forEach(function(J, W) {
								...
								var ne, ae;
								X && (ne = new RTCRtpSender(X, Z.dtlsTransport)), J.wantReceive && (ae = new RTCRtpReceiver(Z.dtlsTransport, Y)), q[W] = {
							...
							return arguments.length && 'function' == typeof arguments[0] && window.setTimeout(arguments[0], 0, H), Promise.resolve(H)
							...
							return arguments.length && 'function' == typeof arguments[0] && window.setTimeout(arguments[0], 0, F), Promise.resolve(F)
							...
							if (!N) this.transceivers.forEach(function(q) {
								...
								if (G) {
									...
									'endOfCandidates' === z.type && (z = {}), G.iceTransport.addRemoteCandidate(z);
				...
				A = {
					shimOnTrack: function() {
						'object' == typeof window && window.RTCPeerConnection && !('ontrack' in window.RTCPeerConnection.prototype) && Object.defineProperty(window.RTCPeerConnection.prototype, 'ontrack', {
							...
							set: function(I) {
								this._ontrack && (this.removeEventListener('track', this._ontrack), this.removeEventListener('addstream', this._ontrackpoly)), this.addEventListener('track', this._ontrack = I), this.addEventListener('addstream', this._ontrackpoly = function(B) {
									B.stream.getTracks().forEach(function(N) {
										var U = new Event('track');
										U.track = N, U.receiver = {
											track: N
										}, U.streams = [B.stream], this.dispatchEvent(U)
									}.bind(this))
								}.bind(this))
							}
						})
						...
						if ('object' == typeof window && (window.RTCPeerConnection || window.mozRTCPeerConnection)) {
							window.RTCPeerConnection || (window.RTCPeerConnection = function(U, F) {
									...
									for (var G = [], z = 0, V; z < U.iceServers.length; z++)
										if (V = U.iceServers[z], V.hasOwnProperty('urls'))
											for (var q = 0, H; q < V.urls.length; q++) H = {
												url: V.urls[q]
											}, 0 === V.urls[q].indexOf('turn') && (H.username = V.username, H.credential = V.credential), G.push(H);
								...
								RTCPeerConnection.prototype[U] = function() {
									return arguments[0] = new('addIceCandidate' === U ? RTCIceCandidate : RTCSessionDescription)(arguments[0]), F.apply(this, arguments)
								}
							});
								...
								if ('require' !== W && 'advanced' !== W && 'mediaSource' !== W) {
									...
									if ((void 0 !== X.min || void 0 !== X.max || void 0 !== X.exact) && J.push(W), void 0 !== X.exact && ('number' == typeof X.exact ? X.min = X.max = X.exact : H[W] = X.exact, delete X.exact), void 0 !== X.ideal) {
										...
										Y[W] = 'number' == typeof X.ideal ? {
											min: X.ideal,
											max: X.ideal
										} : X.ideal, H.advanced.push(Y), delete X.ideal, Object.keys(X).length || delete H[W]
									}
								}
							}), J.length && (H.require = J), H
						};
						return G = JSON.parse(JSON.stringify(G)), 38 > A.version && (M('spec: ' + JSON.stringify(G)), G.audio && (G.audio = q(G.audio)), G.video && (G.video = q(G.video)), M('ff37: ' + JSON.stringify(G))), navigator.mozGetUserMedia(G, z, function(H) {
							V(I(H))
						})
					};
				if (navigator.mediaDevices || (navigator.mediaDevices = {
			...
			var M = !0,
				A = {
					...
					detectBrowser: function() {
						...
						if ('undefined' == typeof window || !window.navigator) return I.browser = 'Not a browser.', I;
						if (navigator.mozGetUserMedia) I.browser = 'firefox', I.version = this.extractVersion(navigator.userAgent, /Firefox\/([0-9]+)\./, 1);
						else if (navigator.webkitGetUserMedia) {
							if (window.webkitRTCPeerConnection) I.browser = 'chrome', I.version = this.extractVersion(navigator.userAgent, /Chrom(e|ium)\/([0-9]+)\./, 2);
							else if (navigator.userAgent.match(/Version\/(\d+).(\d+)/)) I.browser = 'safari', I.version = this.extractVersion(navigator.userAgent, /AppleWebKit\/([0-9]+)\./, 1);
							else return I.browser = 'Unsupported webkit-based browser with GUM support but no WebRTC support.', I;
						} else if (navigator.mediaDevices && navigator.userAgent.match(/Edge\/(\d+).(\d+)$/)) I.browser = 'edge', I.version = this.extractVersion(navigator.userAgent, /Edge\/(\d+).(\d+)$/, 2);
						else return I.browser = 'Not a supported browser.', I;
						return I
					}
				};
			R.exports = {
		...
		13: [function(P, R) {
			...
			R.exports = M, M.prototype.duration = function() {
			...
			(function(M) {
				...
				D.encode = function(A) {
					var I = new Uint8Array(A),
						...
						U = '',
						B;
					for (B = 0; B < N; B += 3) U += M[I[B] >> 2], U += M[(3 & I[B]) << 4 | I[B + 1] >> 4], U += M[(15 & I[B + 1]) << 2 | I[B + 2] >> 6], U += M[63 & I[B + 2]];
					return 2 == N % 3 ? U = U.substring(0, U.length - 1) + '=' : 1 == N % 3 && (U = U.substring(0, U.length - 2) + '=='), U
				}, D.decode = function(A) {
					var I = 0.75 * A.length,
						B = A.length,
						U = 0,
					...
					var q = new ArrayBuffer(I),
						H = new Uint8Array(q);
					for (N = 0; N < B; N += 4) F = M.indexOf(A[N]), G = M.indexOf(A[N + 1]), z = M.indexOf(A[N + 2]), V = M.indexOf(A[N + 3]), H[U++] = F << 2 | G >> 4, H[U++] = (15 & G) << 4 | z >> 2, H[U++] = (3 & z) << 6 | 63 & V;
					return q
				}
					...
					for (var V = 0, q; V < z.length; V++)
						if (q = z[V], q.buffer instanceof ArrayBuffer) {
							...
							if (q.byteLength !== H.byteLength) {
								...
								J.set(new Uint8Array(H, q.byteOffset, q.byteLength)), H = J.buffer
							}
							z[V] = H
						}
				}
		...
		17: [function(P, R) {
			...
			M.prototype.on = M.prototype.addEventListener = function(I, B) {
				return this._callbacks = this._callbacks || {}, (this._callbacks[I] = this._callbacks[I] || []).push(B), this
			}, M.prototype.once = function(I, B) {
				function N() {
					U.off(I, N), B.apply(this, arguments)
				}
				var U = this;
				return this._callbacks = this._callbacks || {}, N.fn = B, this.on(I, N), this
		...
		18: [function(P, R) {
			R.exports = function(M, A) {
				var I = function() {};
				I.prototype = A.prototype, M.prototype = new I, M.prototype.constructor = M
			}
		}, {}],
		19: [function(P, R) {
			function M(I) {
				return M.enabled(I) ? function(B) {
					...
					var N = new Date,
						U = N - (M[I] || N);
					M[I] = N, B = I + ' ' + B + ' +' + M.humanize(U), window.console && console.log && Function.prototype.apply.call(console.log, console, arguments)
				} : function() {}
			}
			...
			R.exports = M, M.names = [], M.skips = [], M.enable = function(I) {
				try {
		...
		21: [function(P, R) {
			R.exports = P('./socket'), R.exports.parser = P('engine.io-parser')
		}, {
			'./socket': 22,
			'engine.io-parser': 33
		}],
		22: [function(P, R) {
			(function(M) {
				...
				var N = P('./transports'),
					...
					q = P('parsejson'),
					H = P('parseqs');
				R.exports = I, I.priorWebsocketSuccess = !1, U(I.prototype), I.protocol = z.protocol, I.Socket = I, I.Transport = P('./transport'), I.transports = P('./transports'), I.parser = P('engine.io-parser'), I.prototype.createTransport = function(J) {
					F('creating transport "%s"', J);
					var W = B(this.query);
					W.EIO = z.protocol, W.transport = J, this.id && (W.sid = this.id);
					var X = new N[J]({
						agent: this.agent,
					...
					var W = this;
					this.transport && (F('clearing existing transport %s', this.transport.name), this.transport.removeAllListeners()), this.transport = J, J.on('drain', function() {
						W.onDrain()
						...
						oe || (F('probe transport "%s" opened', J), ae.send([{
							...
							if (!oe)
								if ('pong' == de.type && 'probe' == de.data) {
									...
									I.priorWebsocketSuccess = 'websocket' == ae.name, F('pausing current transport "%s"', ie.transport.name), ie.transport.pause(function() {
									...
									ce.transport = ae.name, ie.emit('upgradeError', ce)
								}
						}))
					}
					...
					function Y(se) {
						...
						de.transport = ae.name, X(), F('probe transport "%s" failed because of error: %s', J, se), ie.emit('upgradeError', de)
					}
					...
					function ne() {
						ae.removeListener('open', W), ae.removeListener('error', Y), ae.removeListener('close', K), ie.removeListener('close', Z), ie.removeListener('upgrading', Q)
					}
					...
					var ae = this.createTransport(J, {
						...
						oe = !1,
						ie = this;
					I.priorWebsocketSuccess = !1, ae.once('open', W), ae.once('error', Y), ae.once('close', K), this.once('close', Z), this.once('upgrading', Q), ae.open()
					...
					if ('opening' == this.readyState || 'open' == this.readyState) switch (F('socket receive: type "%s", data "%s"', J.type, J.data), this.emit('packet', J), this.emit('heartbeat'), J.type) {
						...
						case 'error':
							...
							W.code = J.data, this.emit('error', W);
							break;
						case 'message':
							this.emit('data', J.data), this.emit('message', J.data);
					} else F('packet received with socket readyState "%s"', this.readyState)
				}, I.prototype.onHandshake = function(J) {
					this.emit('handshake', J), this.id = J.sid, this.transport.query.sid = J.sid, this.upgrades = this.filterUpgrades(J.upgrades), this.pingInterval = J.pingInterval, this.pingTimeout = J.pingTimeout, this.onOpen();
					...
					clearTimeout(J.pingIntervalTimer), J.pingIntervalTimer = setTimeout(function() {
						F('writing ping packet - expecting pong within %sms', J.pingTimeout), J.ping(), J.onHeartbeat(J.pingTimeout)
					}, J.pingInterval)
					...
					return this.sendPacket('message', J, W), this
					...
					if ('closing' != this.readyState && 'closed' != this.readyState) {
						...
						this.emit('packetCreate', Y), this.writeBuffer.push(Y), this.callbackBuffer.push(X), this.flush()
					}
				}, I.prototype.close = function() {
					if ('opening' == this.readyState || 'open' == this.readyState) {
						function W() {
							J.onClose('forced close'), F('socket closing - telling transport to close'), J.transport.close()
						}

						function X() {
							J.removeListener('upgrade', X), J.removeListener('upgradeError', X), W()
						}

						function Y() {
							J.once('upgrade', X), J.once('upgradeError', X)
						}
					...
					F('socket error %j', J), I.priorWebsocketSuccess = !1, this.emit('error', J), this.onClose('transport error', J)
					...
					if ('opening' == this.readyState || 'open' == this.readyState || 'closing' == this.readyState) {
						...
						clearTimeout(this.pingIntervalTimer), clearTimeout(this.pingTimeoutTimer), setTimeout(function() {
							X.writeBuffer = [], X.callbackBuffer = [], X.prevBufferLen = 0
						}, 0), this.transport.removeAllListeners('close'), this.transport.close(), this.transport.removeAllListeners(), this.readyState = 'closed', this.id = null, this.emit('close', J, W)
					}
				...
				D.polling = function(F) {
					...
					if (M.location) {
						var H = 'https:' == location.protocol,
							J = location.port;
						J || (J = H ? 443 : 80), z = F.hostname != location.hostname || J != F.port, V = F.secure != H
					}
		...
		25: [function(P, R) {
			(function(M) {
				...
				function I(V) {
					B.call(this, V), this.query = this.query || {}, G || (!M.___eio && (M.___eio = []), G = M.___eio), this.index = G.length;
					var q = this;
					G.push(function(H) {
						q.onData(H)
					}), this.query.j = this.index, M.document && M.addEventListener && M.addEventListener('beforeunload', function() {
				...
				var U = /\n/g,
					F = /\\n/g,
					G;
				N(I, B), I.prototype.supportsBinary = !1, I.prototype.doClose = function() {
					this.script && (this.script.parentNode.removeChild(this.script), this.script = null), this.form && (this.form.parentNode.removeChild(this.form), this.form = null, this.iframe = null), B.prototype.doClose.call(this)
				}, I.prototype.doPoll = function() {
					var V = this,
						q = document.createElement('script');
					this.script && (this.script.parentNode.removeChild(this.script), this.script = null), q.async = !0, q.src = this.uri(), q.onerror = function(W) {
						V.onError('jsonp poll error', W)
					};
					var H = document.getElementsByTagName('script')[0];
					H.parentNode.insertBefore(q, H), this.script = q;
					...
					function J() {
						...
						try {
							...
							Z = document.createElement('iframe'), Z.name = W.iframeId, Z.src = 'javascript:0'
						}
						Z.id = W.iframeId, W.form.appendChild(Z), W.iframe = Z
					}
					var W = this;
					if (!this.form) {
						var X = document.createElement('form'),
							...
							Z;
						X.className = 'socketio', X.style.position = 'absolute', X.style.top = '-1000px', X.style.left = '-1000px', X.target = K, X.method = 'POST', X.setAttribute('accept-charset', 'utf-8'), Y.name = 'd', X.appendChild(Y), document.body.appendChild(X), this.form = X, this.area = Y
					}
					this.form.action = this.uri(), J(), V = V.replace(F, '\\\n'), this.area.value = V.replace(U, '\\n');
					try {
						this.form.submit()
					} catch (Q) {}
		...
		26: [function(P, R) {
			(function(M) {
				...
				R.exports = I, R.exports.Request = B, z(I, F), I.prototype.supportsBinary = !0, I.prototype.request = function(q) {
					return q = q || {}, q.uri = this.uri(), q.xd = this.xd, q.xs = this.xs, q.agent = this.agent || !1, q.supportsBinary = this.supportsBinary, q.enablesXDR = this.enablesXDR, q.pfx = this.pfx, q.key = this.key, q.passphrase = this.passphrase, q.cert = this.cert, q.ca = this.ca, q.ciphers = this.ciphers, q.rejectUnauthorized = this.rejectUnauthorized, new B(q)
					...
					var W = this.request({
						...
						X = this;
					W.on('success', H), W.on('error', function(Y) {
				...
				}, G(B.prototype), B.prototype.create = function() {
					var q = {
							agent: this.agent,
							xdomain: this.xd,
							xscheme: this.xs,
					...
					try {
						...
						}, V('xhr data %s', this.data), H.send(this.data)
					} catch (W) {
						return void setTimeout(function() {
							J.onError(W)
						}, 0)
					...
					if ('undefined' != typeof this.xhr && null !== this.xhr) {
						...
						M.document && delete B.requests[this.index], this.xhr = null
					}
		...
		27: [function(P, R) {
			function M(G) {
				...
				(!F || z) && (this.supportsBinary = !1), A.call(this, G)
			}
			var A = P('../transport'),
				I = P('parseqs'),
			...
			N(M, A), M.prototype.name = 'polling', M.prototype.doOpen = function() {
				...
				function G() {
					U('writing close packet'), z.write([{
						type: 'close'
					}])
				}
				var z = this;
				...
				var G = this.query || {},
					...
					V = '';
				return !1 !== this.timestampRequests && (G[this.timestampParam] = +new Date + '-' + A.timestamps++), this.supportsBinary || G.sid || (G.b64 = 1), G = I.encode(G), this.port && ('https' == z && 443 != this.port || 'http' == z && 80 != this.port) && (V = ':' + this.port), G.length && (G = '?' + G), z + '://' + this.hostname + V + this.path + G
			}
		}, {
			'../transport': 23,
			'component-inherit': 18,
		...
		28: [function(P, R) {
			function M(G) {
				var z = G && G.forceBase64;
				z && (this.supportsBinary = !1), A.call(this, G)
			}
			var A = P('../transport'),
				...
				F = P('ws');
			R.exports = M, N(M, A), M.prototype.name = 'websocket', M.prototype.supportsBinary = !0, M.prototype.doOpen = function() {
				if (this.check()) {
					var G = this,
						z = this.uri(),
						q = {
						...
						V;
					q.pfx = this.pfx, q.key = this.key, q.passphrase = this.passphrase, q.cert = this.cert, q.ca = this.ca, q.ciphers = this.ciphers, q.rejectUnauthorized = this.rejectUnauthorized, this.ws = new F(z, V, q), void 0 === this.ws.binaryType && (this.supportsBinary = !1), this.ws.binaryType = 'arraybuffer', this.addEventListeners()
				}
				...
				var G = this.query || {},
					...
					V = '';
				return this.port && ('wss' == z && 443 != this.port || 'ws' == z && 80 != this.port) && (V = ':' + this.port), this.timestampRequests && (G[this.timestampParam] = +new Date), this.supportsBinary || (G.b64 = 1), G = B.encode(G), G.length && (G = '?' + G), z + '://' + this.hostname + V + this.path + G
		...
		30: [function(P, R, D) {
			...
			D = R.exports = P('./debug'), D.log = function() {
			...
			}, D.colors = ['lightseagreen', 'forestgreen', 'goldenrod', 'dodgerblue', 'darkorchid', 'crimson'], D.formatters.j = function(U) {
			...
			D = R.exports = function(z) {
				...
				function q() {
					var J = q,
						W = +new Date,
						X = W - (G || W);
					J.diff = X, J.prev = G, J.curr = W, G = W, null == J.useColors && (J.useColors = D.useColors()), null == J.color && J.useColors && (J.color = M());
				...
				V.enabled = !1, q.enabled = !0;
				var H = D.enabled(z) ? q : V;
				return H.namespace = z, H
			}, D.coerce = function(z) {
				...
				return !1
			}, D.humanize = P('ms'), D.names = [], D.skips = [], D.formatters = {};
			var F = 0,
				G
		}, {
			ms: 47
		...
		32: [function(P, R) {
			...
			R.exports = function(B) {
				...
				return -1 != U && -1 != F && (z.source = N, z.host = z.host.substring(1, z.host.length - 1).replace(/;/g, ':'), z.authority = z.authority.replace('[', '').replace(']', '').replace(/;/g, ':'), z.ipv6uri = !0), z
			}
		}, {}],
		33: [function(P, R, D) {
			(function(M) {
				...
				function B(ne, ae, oe) {
					...
					return ie.onload = function() {
						ne.data = ie.result, D.encodePacket(ne, ae, !0, oe)
				...
				D.encodePacket = function(ne, ae, oe, ie) {
					...
					var de = Y[ne.type];
					return void 0 !== ne.data && (de += oe ? H.encode(ne.data + '') : ne.data + ''), ie('' + de)
					...
					return ne.length ? void U(ne, function(ie, se) {
						...
						var de = se.reduce(function(le, ue) {
								var me;
								return me = 'string' == typeof ue ? ue.length : ue.byteLength, le + me.toString().length + me + 2
							}, 0),
					...
					for (var ie = ne, se = [], de = !1; 0 < ie.byteLength;) {
						...
						ie = z(ie, 2 + le.length), le = parseInt(le);
						var me = z(ie, 0, le);
						if (pe) try {
		...
		35: [function(P, R) {
			...
			var I = P('wildemitter'),
				B = P('util');
			B.inherits(M, I), M.prototype.send = function(N, U) {
				var F = this;
				this.file = N, this.channel = U;
				var G = 'number' != typeof U.bufferedAmountLowThreshold,
					...
					V = function() {
						...
						q.onload = function() {
							return function(J) {
								F.channel.send(J.target.result), F.emit('progress', z, N.size, J.target.result), N.size > z + J.target.result.byteLength ? G ? window.setTimeout(V, F.config.pacing) : U.bufferedAmount <= U.bufferedAmountLowThreshold && window.setTimeout(V, 0) : (F.emit('progress', N.size, N.size, null), F.emit('sentFile')), z += F.config.chunksize
							}
						}(N);
						var H = N.slice(z, z + F.config.chunksize);
						q.readAsArrayBuffer(H)
					};
				G || (U.bufferedAmountLowThreshold = 8 * this.config.chunksize, U.addEventListener('bufferedamountlow', V)), window.setTimeout(V, 0)
			}, B.inherits(A, I), A.prototype.receive = function(N, U) {
				var F = this;
				N && (this.metadata = N), this.channel = U, U.binaryType = 'arraybuffer', this.channel.onmessage = function(G) {
					var z = G.data.byteLength;
					F.received += z, F.receiveBuffer.push(G.data), F.emit('progress', F.received, F.metadata.size, G.data), F.received === F.metadata.size ? (F.emit('receivedFile', new window.Blob(F.receiveBuffer), F.metadata), F.receiveBuffer = []) : F.received > F.metadata.size && (console.error('received more than expected, discarding...'), F.receiveBuffer = [])
				}
			}, R.exports = {}, R.exports.support = 'undefined' != typeof window && window && window.File && window.FileReader && window.Blob, R.exports.Sender = M, R.exports.Receiver = A
		}, {
			util: 73,
			wildemitter: 75
		}],
		36: [function(P, R) {
			...
			R.exports = function(A, I) {
				var B = 2 === arguments.length,
					N = B ? I : A,
					U;
				if ('undefined' == typeof window || 'http:' === window.location.protocol) return U = new Error('NavigatorUserMediaError'), U.name = 'HTTPS_REQUIRED', N(U);
				if (window.navigator.userAgent.match('Chrome')) {
					...
					if (window.navigator.userAgent.match('Linux') && (G = 35), sessionStorage.getScreenMediaJSExtensionId) chrome.runtime.sendMessage(sessionStorage.getScreenMediaJSExtensionId, {
						...
						}, A.video.mandatory.chromeMediaSourceId = H.sourceId, window.navigator.mediaDevices.getUserMedia(A).then(function(W) {
							N(null, W)
						}).catch(function(W) {
							N(W)
						})
						...
						if (!H) {
							...
							J.name = 'CEF_GETSCREENMEDIA_CANCELED', N(J)
						} else A = B && A || {
							audio: !1,
							video: {
								mandatory: {
						...
						}, A.video.mandatory.chromeMediaSourceId = H, window.navigator.mediaDevices.getUserMedia(A).then(function(W) {
							N(null, W)
						}).catch(function(W) {
							N(W)
						})
						...
						var V = window.setTimeout(function() {
							return U = new Error('NavigatorUserMediaError'), U.name = 'EXTENSION_UNAVAILABLE', N(U)
						}, 1e3);
						M[V] = [N, B ? A : null], window.postMessage({
							type: 'getScreen',
							id: V
						}, '*')
					}
					...
					33 <= q ? (A = B && A || {
						...
						var J = H.currentTime,
							W = window.setInterval(function() {
								H || window.clearInterval(W), H.currentTime == J && (window.clearInterval(W), H.onended && H.onended()), J = H.currentTime
							}, 500)
					}).catch(function(H) {
						N(H)
				...
				if (A.origin == window.location.origin)
					if ('gotScreen' == A.data.type && M[A.data.id]) {
						...
						}, B.video.mandatory.chromeMediaSourceId = A.data.sourceId, window.navigator.mediaDevices.getUserMedia(B).then(function(F) {
							N(null, F)
						}).catch(function(F) {
							N(F)
						})
		...
		38: [function(P, R) {
			...
			R.exports = function(N, U) {
				...
				B || (B = new I);
				var W, X, Y;
				Y = B.createAnalyser(), Y.fftSize = 512, Y.smoothingTimeConstant = G, X = new Float32Array(Y.frequencyBinCount), N.jquery && (N = N[0]), N instanceof HTMLAudioElement || N instanceof HTMLVideoElement ? (W = B.createMediaElementSource(N), 'undefined' == typeof q && (q = !0), V = V || -50) : (W = B.createMediaStreamSource(N), V = V || -50), W.connect(Y), q && Y.connect(B.destination), F.speaking = !1, F.setThreshold = function(Q) {
					V = Q
				}, F.setInterval = function(Q) {
					z = Q
				}, F.stop = function() {
					J = !1, F.emit('volume_change', -100, V), F.speaking && (F.speaking = !1, F.emit('stopped_speaking')), Y.disconnect(), W.disconnect()
				}, F.speakingHistory = [];
				...
				var Z = function() {
					setTimeout(function() {
						if (J) {
							...
							F.speakingHistory.shift(), F.speakingHistory.push(0 + (Q > V)), Z()
						}
					}, z)
				};
				return Z(), F
				...
				R.exports = function(B) {
					function N(U) {
						...
						if (I(U)) {
							...
							for (var G in U.toJSON && (U = U.toJSON()), U)
				...
				if (!A('json')) {
					...
					(B = {}.hasOwnProperty) || (B = function(Ce) {
						...
						return (Se.__proto__ = null, Se.__proto__ = {
							...
							return this.__proto__ = we, Ee
						}, Se = null, B.call(this, Ce)
					});
					var ae = {
							boolean: 1,
							number: 1,
					...
					if (N = function(Ce, Se) {
							...
							return Te = we = null, xe ? 2 == xe ? N = function(Pe, Re) {
								var _e = {},
									De = I.call(Pe) == q,
									Me;
							...
							le = function(Ce, Se, xe, Te, we, Ee, Pe) {
								...
								if ('object' == typeof Re && Re)
									...
									if (ne) {
										...
										je = 1 + je - ne(De, Me), Oe = (Re % 864e5 + 864e5) % 864e5, Le = Z(Oe / 36e5) % 24, Ae = Z(Oe / 6e4) % 60, Ie = Z(Oe / 1e3) % 60, Be = Oe % 1e3
									} else De = Re.getUTCFullYear(), Me = Re.getUTCMonth(), je = Re.getUTCDate(), Le = Re.getUTCHours(), Ae = Re.getUTCMinutes(), Ie = Re.getUTCSeconds(), Be = Re.getUTCMilliseconds();
								...
								if ('object' == typeof Re) {
									...
									if (Pe.push(Re), Ne = [], ze = Ee, Ee += we, _e == X) {
										for (Fe = 0, Ge = Re.length; Fe < Ge; Fe++) Ue = le(Fe, Re, xe, Te, we, Ee, Pe), Ne.push(Ue == U ? 'null' : Ue);
										Ve = Ne.length ? we ? '[\n' + Ee + Ne.join(',\n' + Ee) + '\n' + ze + ']' : '[' + Ne.join(',') + ']' : '[]'
					...
					if (!A('json-parse')) {
						var ue = String.fromCharCode,
							...
							ye = function() {
								for (var Ce = ge, Se = Ce.length, xe, Te, we, Ee, Pe; fe < Se;) switch (Pe = Ce.charCodeAt(fe), Pe) {
									...
									case 58:
									case 44:
										return xe = K ? Ce.charAt(fe) : Ce[fe], fe++, xe;
									case 34:
										for (xe = '@', fe++; fe < Se;)
											...
											case 114:
												...
												break;
											case 117:
												for (Te = ++fe, we = fe + 4; fe < we; fe++) Pe = Ce.charCodeAt(fe), 48 <= Pe && 57 >= Pe || 97 <= Pe && 102 >= Pe || 65 <= Pe && 70 >= Pe || he();
										...
										if (34 == Ce.charCodeAt(fe)) return fe++, xe;
										he();
									default:
										...
										if ('null' == Ce.slice(fe, fe + 4)) return fe += 4, null;
										he();
								}
								return '$'
							},
							be = function(Ce) {
								...
								if ('$' == Ce && he(), 'string' == typeof Ce) {
									...
									if ('[' == Ce) {
										for (Se = [];; xe || (xe = !0)) {
											if (Ce = ye(), ']' == Ce) break;
											xe && (',' == Ce ? (Ce = ye(), ']' == Ce && he()) : he()), ',' == Ce && he(), Se.push(be(Ce))
										}
										return Se
									}
									if ('{' == Ce) {
										for (Se = {};; xe || (xe = !0)) {
											if (Ce = ye(), '}' == Ce) break;
											xe && (',' == Ce ? (Ce = ye(), '}' == Ce && he()) : he()), (',' == Ce || 'string' != typeof Ce || '@' != (K ? Ce.charAt(0) : Ce[0]) || ':' != ye()) && he(), Se[Ce.slice(1)] = be(ye())
										}
										return Se
									}
									he()
						...
						z.parse = function(Ce, Se) {
							var xe, Te;
							return fe = 0, ge = '' + Ce, xe = be(ye()), '$' != ye() && he(), fe = ge = null, Se && I.call(Se) == q ? ke((Te = {}, Te[''] = xe, Te), '', Se) : xe
						}
					}
				}
				F && C(function() {
		...
		44: [function(P, R) {
			...
			var B = P('util'),
				...
				F = P('wildemitter'),
				G = P('mockconsole');
			B.inherits(I, F), I.prototype.start = function(z, V) {
				var q = this,
					H = z || this.config.media;
				this.emit('localStreamRequested', H), navigator.mediaDevices.getUserMedia(H).then(function(J) {
				...
				this.stopStream(z), this.stopScreenShare(z)
				...
				var q = this;
				this.emit('localScreenRequested'), 'function' != typeof z || V || (V = z, z = null), U(z, function(H, J) {
			...
			(function(M) {
				...
				function ke(Gr, zr) {
					var Vr = Ur(Gr) || et(Gr) ? F(Gr.length, String) : [],
						qr = Vr.length;
					for (var Jr in Gr)(zr || cr.call(Gr, Jr)) && !(!!qr && ('length' == Jr || We(Jr, qr))) && Vr.push(Jr);
					return Vr
				}
				...
				function Te(Gr, zr, Vr, qr, Hr, Jr, Wr) {
					...
					return B(Qr || Gr, function(en, tn) {
						Qr && (tn = en, en = Gr[tn]), Ce(Xr, tn, Te(en, zr, Vr, qr, tn, Gr, Wr))
					}), Xr
				}
				...
				function De(Gr, zr) {
					...
					return Gr.copy(Vr), Vr
				}

				function Me(Gr) {
					var zr = new Gr.constructor(Gr.byteLength);
					return new gr(zr).set(new gr(Gr)), zr
				}
				...
				function Le(Gr) {
					...
					return zr.lastIndex = Gr.lastIndex, zr
				}
				...
				function qe(Gr) {
					...
					return zr && 'string' == typeof Gr[0] && cr.call(Gr, 'index') && (Vr.index = Gr.index, Vr.input = Gr.input), Vr
				}
				...
				var mt = '__lodash_hash_undefined__',
					...
					Wt = /^(?:0|[1-9]\d*)$/,
					Xt = {};
				Xt[gt] = Xt['[object Array]'] = Xt[Ot] = Xt[Lt] = Xt[vt] = Xt[kt] = Xt[At] = Xt[It] = Xt[Bt] = Xt[Nt] = Xt[Ut] = Xt[Tt] = Xt[wt] = Xt[Et] = Xt[Rt] = Xt[_t] = Xt[Dt] = Xt[Mt] = Xt[Ft] = Xt[Gt] = Xt[zt] = Xt[Vt] = !0, Xt['[object Error]'] = Xt[St] = Xt[jt] = !1;
				...
				J.prototype.clear = function() {
					...
					return Vr[Gr] = _r && void 0 === zr ? mt : zr, this
					...
					var Vr = this.__data__,
						qr = Se(Vr, Gr);
					return 0 > qr ? Vr.push([Gr, zr]) : Vr[qr][1] = zr, this
					...
					return ze(this, Gr).set(Gr, zr), this
					...
					if (Vr instanceof Q) {
						var qr = Vr.__data__;
						if (!Tr || qr.length < 200 - 1) return qr.push([Gr, zr]), this;
						Vr = this.__data__ = new de(qr)
					}
					return Vr.set(Gr, zr), this
				};
		...
		50: [function(P, R, D) {
			D.encode = function(M) {
				...
				for (var A = {}, I = M.split('&'), B = 0, N = I.length, U; B < N; B++) U = I[B].split('='), A[decodeURIComponent(U[0])] = decodeURIComponent(U[1]);
				return A
			}
		}, {}],
		51: [function(P, R) {
		...
		52: [function(P, R) {
			...
			function I(Y) {
				...
				if ((V === M || !V) && setTimeout) return V = setTimeout, setTimeout(Y, 0);
				try {
					return V(Y, 0)
				} catch (K) {
					try {
			...
			function B(Y) {
				...
				if ((q === A || !q) && clearTimeout) return q = clearTimeout, clearTimeout(Y);
				try {
					return q(Y)
				} catch (K) {
					try {
			...
			z.nextTick = function(Y) {
				...
				H.push(new F(Y, K)), 1 !== H.length || J || I(U)
			}, F.prototype.run = function() {
				this.fun.apply(null, this.array)
			}, z.title = 'browser', z.browser = !0, z.env = {}, z.argv = [], z.version = '', z.versions = {}, z.on = G, z.addListener = G, z.once = G, z.off = G, z.removeListener = G, z.removeAllListeners = G, z.emit = G, z.prependListener = G, z.prependOnceListener = G, z.listeners = function() {
				return []
			}, z.binding = function() {
		...
		53: [function(P, R) {
			...
			A.inherits(M, B), Object.defineProperty(M.prototype, 'signalingState', {
				get: function() {
				...
				this.localStream = U, this.pc.addStream(U)
				...
				if (U.contents || U.jingle && U.jingle.contents) {
					...
					V.forEach(function(q) {
						...
						if (!(G.iceCredentials.remote[q.name] && H.ufrag && G.iceCredentials.remote[q.name].ufrag !== H.ufrag)) K();
							...
							Y.transport.ufrag = H.ufrag, Y.transport.pwd = H.pwd;
							var Z = {
								type: 'offer',
					...
					if (U.candidate && 0 !== U.candidate.candidate.indexOf('a=') && (U.candidate.candidate = 'a=' + U.candidate.candidate), this.wtFirefox && null !== this.firefoxcandidatebuffer && this.pc.localDescription && 'offer' === this.pc.localDescription.type) return this.firefoxcandidatebuffer.push(U.candidate), F();
					G.pc.addIceCandidate(new RTCIceCandidate(U.candidate), function() {}, function(q) {
						G.emit('error', q)
					}), G._checkRemoteCandidate(U.candidate.candidate), F()
				}
				...
				return F = z ? F : U, F = F || function() {}, 'closed' === this.pc.signalingState ? F('Already closed') : void this.pc.createOffer(function(q) {
					var H = {
						type: 'offer',
						sdp: q.sdp
					};
					G.assumeSetLocalSuccess && (G.emit('offer', H), F(null, H)), G._candidateBuffer = [], G.pc.setLocalDescription(q, function() {
						var J;
				...
				return 'closed' === this.pc.signalingState ? F('Already closed') : void G.pc.createAnswer(function(z) {
					...
					if (G.enableChromeNativeSimulcast && (z.jingle = I.toSessionJSON(z.sdp, {
						...
						if (q.forEach(function(K) {
							...
							J.ssrc = '' + Math.floor(4294967295 * Math.random()), z.jingle.contents[1].application.sources.push(J), V.push(z.jingle.contents[1].application.sources[0].ssrc), V.push(J.ssrc), q.push({
								semantics: 'SIM',
								sources: V
							});
							var W = JSON.parse(JSON.stringify(J));
							W.ssrc = '' + Math.floor(4294967295 * Math.random()), z.jingle.contents[1].application.sources.push(W), q.push({
								semantics: 'FID',
								sources: [J.ssrc, W.ssrc]
							}), z.jingle.contents[1].application.sourceGroups = q, z.sdp = I.toSessionSDP(z.jingle, {
								sid: G.config.sdpSessionID,
								role: G._role(),
								direction: 'outgoing'
							})
					...
					if (G.assumeSetLocalSuccess) {
						var Y = N(X);
						G.emit('answer', Y), F(null, Y)
					}
					G._candidateBuffer = [], G.pc.setLocalDescription(z, function() {
						if (G.config.useJingle) {
							var K = I.toSessionJSON(z.sdp, {
								role: G._role(),
								direction: 'outgoing'
							});
							K.sid = G.config.sid, G.localDescription = K, X.jingle = K
						}
						if (G.enableChromeNativeSimulcast && (!X.jingle && (X.jingle = I.toSessionJSON(z.sdp, {
								...
								Q.parameters = Q.parameters.map(function(ae) {
									return 'msid' === ae.key && (ae.value += '-' + Math.floor(ne / 2)), ae
								})
					...
					if (this._candidateBuffer.push(V), F.config.useJingle) {
						...
						if (z.jingle = {
							...
							F.batchedIceCandidates.forEach(function(Y) {
								Y = Y.contents[0], W[Y.name] || (W[Y.name] = Y), W[Y.name].transport.candidates.push(Y.transport.candidates[0])
							});
							var X = {
								jingle: {
									contents: []
							...
							}), F.batchedIceCandidates = [], F.emit('ice', X)
						}, F.batchIceCandidates), void F.batchedIceCandidates.push(z.jingle)
					}
					this.emit('ice', z)
				} else this.emit('endOfCandidates')
			}, M.prototype._onDataChannel = function(U) {
				var F = U.channel;
				this._remoteDataChannels.push(F), this.emit('addChannel', F)
		...
		54: [function(P, R, D) {
			...
			}, D.toCandidateSDP = M.toCandidateSDP, D.toMediaSDP = M.toMediaSDP, D.toSessionSDP = M.toSessionSDP, D.toIncomingJSONOffer = function(I, B) {
			...
			}, D.toCandidateJSON = A.toCandidateJSON, D.toMediaJSON = A.toMediaJSON, D.toSessionJSON = A.toSessionJSON
		}, {
			'./lib/tojson': 57,
			'./lib/tosdp': 58
		}],
		55: [function(P, R, D) {
			D.lines = function(M) {
				...
				return A = A[0].split('/'), I.name = A[0], I.clockrate = A[1], I.channels = 3 == A.length ? A[2] : '1', I
				...
				for (var N = M.substr(M.indexOf(' ') + 1).split(';'), U = [], F = 0, A, I, B; F < N.length; F++) A = N[F].split('='), I = A[0].trim(), B = A[1], I && B ? U.push({
					key: I,
					value: B
				}) : I && U.push({
					key: '',
				...
				var A = M.substr(9).split(' '),
					...
					B = A.shift(),
					N = B.indexOf('/');
				return 0 <= N ? (I.id = B.substr(0, N), I.senders = B.substr(N + 1)) : (I.id = B, I.senders = 'sendrecv'), I.uri = A.shift() || '', I
			}, D.rtcpfb = function(M) {
				var A = M.substr(10).split(' '),
					I = {};
				return I.id = A.shift(), I.type = A.shift(), 'trr-int' === I.type ? I.value = A.shift() : I.subtype = A.shift() || '', I.parameters = A, I
				...
				return I.network = '1', I
			}, D.sourceGroups = function(M) {
				for (var A = [], I = 0, B; I < M.length; I++) B = M[I].substr(13).split(' '), A.push({
					semantics: B.shift(),
					sources: B
				});
				return A
				...
				for (var A = [], B = 0, I; B < M.length; B++) I = M[B].substr(8).split(' '), A.push({
					semantics: I.shift(),
					contents: I
				});
				return A
				...
				var A = M.substr(2).split(':'),
					I = {};
				return I.type = A.shift(), I.bandwidth = A.shift(), I
			}, D.msid = function(M) {
				var A = M.substr(7),
					I = A.split(' ');
				return {
		...
		57: [function(P, R, D) {
			...
			D._setIdCounter = function(B) {
				...
				var F = N.creators || [],
					...
					V = B.split('\r\nm='),
					U;
				for (U = 1; U < V.length; U++) V[U] = 'm=' + V[U], U !== V.length - 1 && (V[U] += '\r\n');
				var q = V.shift() + '\r\n',
					H = A.lines(q),
					J = {},
					W = [];
				...
				return X.length && (J.groups = A.groups(X)), J
				...
				if (Y && (J.name = Y.substr(6)), A.findLine('a=sendrecv', V, q) ? J.senders = 'both' : A.findLine('a=sendonly', V, q) ? J.senders = M[G][z].sendonly : A.findLine('a=recvonly', V, q) ? J.senders = M[G][z].recvonly : A.findLine('a=inactive', V, q) && (J.senders = 'none'), 'rtp' == W.applicationType) {
					...
					oe.forEach(function(ye) {
						var be = A.extmap(ye);
						be.senders = M[G][z][be.senders], W.headerExtensions.push(be)
					});
				...
				le.forEach(function(ye) {
					...
					ue && (be.setup = ue.substr(8)), X.fingerprints.push(be)
				});
				var me = A.findLine('a=ice-ufrag:', V, q),
					fe = A.findLine('a=ice-pwd:', V, q);
				if (me && fe) {
					X.ufrag = me.substr(12), X.pwd = fe.substr(10), X.candidates = [];
				...
				return N.id = (I++).toString(36).substr(0, 12), N
			}
		}, {
			'./parsers': 55,
			'./senders': 56
		...
		58: [function(P, R, D) {
			...
			D.toSessionSDP = function(A, I) {
				...
				if ('datachannel' == F.applicationType ? (q.push('application'), q.push('1'), q.push('DTLS/SCTP'), G.sctp && G.sctp.forEach(function(Z) {
					...
					G.ufrag && B.push('a=ice-ufrag:' + G.ufrag), G.pwd && B.push('a=ice-pwd:' + G.pwd);
					V.forEach(function(Z) {
						B.push('a=fingerprint:' + Z.hash + ' ' + Z.value), Z.setup && !!1 && B.push('a=setup:' + Z.setup)
					}), G.sctp && G.sctp.forEach(function(Z) {
						B.push('a=sctpmap:' + Z.number + ' ' + Z.protocol + ' ' + Z.streams)
					})
				}
				'rtp' == F.applicationType && B.push('a=' + (M[N][U][A.senders] || 'sendrecv')), B.push('a=mid:' + A.name), F.sources && F.sources.length && (F.sources[0].parameters || []).forEach(function(Z) {
				...
				J.forEach(function(Z) {
				...
				}), F.googConferenceFlag && B.push('a=x-google-flag:conference'), z.forEach(function(Z) {
					...
					if (Z.channels && '1' != Z.channels && (Q += '/' + Z.channels), B.push(Q), Z.parameters && Z.parameters.length) {
						...
						}), ne.push(ae.join(';')), B.push(ne.join(' '))
					}
				...
				Y.forEach(function(Z) {
					for (var Q = 0, ne; Q < Z.parameters.length; Q++) ne = Z.parameters[Q], B.push('a=ssrc:' + (Z.ssrc || F.ssrc) + ' ' + ne.key + (ne.value ? ':' + ne.value : ''))
				});
				var K = G.candidates || [];
				...
				var I = [A.foundation, A.component, A.protocol.toUpperCase(), A.priority, A.ip, A.port],
					B = A.type;
				return I.push('typ'), I.push(B), ('srflx' === B || 'prflx' === B || 'relay' === B) && A.relAddr && A.relPort && (I.push('raddr'), I.push(A.relAddr), I.push('rport'), I.push(A.relPort)), A.tcpType && 'TCP' == A.protocol.toUpperCase() && (I.push('tcptype'), I.push(A.tcpType)), I.push('generation'), I.push(A.generation || '0'), 'a=candidate:' + I.join(' ')
			}
		}, {
			'./senders': 56
		}],
		...
		61: [function(P, R, D) {
			function M(F, G) {
				...
				var z = A(F),
					...
					q = z.id,
					H;
				return G.forceNew || G['force new connection'] || !1 === G.multiplex ? (N('ignoring socket cache for %s', V), H = B(V, G)) : (!U[q] && (N('new io instance for %s', V), U[q] = B(V, G)), H = U[q]), H.socket(z.path)
			}
			var A = P('./url'),
			...
			R.exports = D = M;
			var U = D.managers = {};
			D.protocol = I.protocol, D.connect = M, D.Manager = P('./manager'), D.Socket = P('./socket')
		}, {
			'./manager': 62,
			'./socket': 64,
			'./url': 65,
		...
		62: [function(P, R) {
			...
			var A = P('./url'),
				...
				q = P('indexof'),
				H = P('backo2');
			R.exports = M, M.prototype.emitAll = function() {
				for (var J in this.emit.apply(this, arguments), this.nsps) this.nsps[J].emit.apply(this.nsps[J], arguments)
			}, M.prototype.updateSocketIds = function() {
				for (var J in this.nsps) this.nsps[J].id = this.engine.id
			}, N(M.prototype), M.prototype.reconnection = function(J) {
				...
				V('opening %s', this.uri), this.engine = I(this.uri, this.opts);
				var W = this.engine,
					X = this;
				...
				if (!1 !== this._timeout) {
					...
					var Q = setTimeout(function() {
						V('connect attempt timed out after %d', Z), Y.destroy(), W.close(), W.emit('error', 'timeout'), X.emitAll('connect_timeout', Z)
					}, Z);
					this.subs.push({
						destroy: function() {
							clearTimeout(Q)
				...
				return this.subs.push(Y), this.subs.push(K), this
			}, M.prototype.onopen = function() {
				V('open'), this.cleanup(), this.readyState = 'open', this.emit('open');
				var J = this.engine;
				this.subs.push(F(J, 'data', G(this, 'ondata'))), this.subs.push(F(this.decoder, 'decoded', G(this, 'ondecoded'))), this.subs.push(F(J, 'error', G(this, 'onerror'))), this.subs.push(F(J, 'close', G(this, 'onclose')))
			}, M.prototype.ondata = function(J) {
				this.decoder.add(J)
			}, M.prototype.ondecoded = function(J) {
				this.emit('packet', J)
			}, M.prototype.onerror = function(J) {
				V('error', J), this.emitAll('error', J)
			}, M.prototype.socket = function(J) {
				var W = this.nsps[J];
				if (!W) {
					W = new B(this, J), this.nsps[J] = W;
					var X = this;
				...
				W.encoding ? W.packetBuffer.push(J) : (W.encoding = !0, this.encoder.encode(J, function(X) {
					...
					W.encoding = !1, W.processPacketQueue()
				}))
				...
				V('close'), this.cleanup(), this.backoff.reset(), this.readyState = 'closed', this.emit('close', J), this._reconnection && !this.skipReconnect && this.reconnect()
			}, M.prototype.reconnect = function() {
				if (this.reconnecting || this.skipReconnect) return this;
				var J = this;
				if (this.backoff.attempts >= this._reconnectionAttempts) V('reconnect failed'), this.backoff.reset(), this.emitAll('reconnect_failed'), this.reconnecting = !1;
				else {
					var W = this.backoff.duration();
					V('will wait %dms before reconnect attempt', W), this.reconnecting = !0;
				...
				this.reconnecting = !1, this.backoff.reset(), this.updateSocketIds(), this.emitAll('reconnect', J)
			}
		}, {
			'./on': 63,
			'./socket': 64,
		...
		64: [function(P, R, D) {
			function M(q, H) {
				this.io = q, this.nsp = H, this.json = this, this.ids = 0, this.acks = {}, this.io.autoConnect && this.open(), this.receiveBuffer = [], this.sendBuffer = [], this.connected = !1, this.disconnected = !0
			}
			...
			var z = {
				...
				V = I.prototype.emit;
			I(M.prototype), M.prototype.subEvents = function() {
				if (!this.subs) {
					var q = this.io;
				...
				if (z.hasOwnProperty(q)) return V.apply(this, arguments), this;
				var H = B(arguments),
					J = A.EVENT;
				...
				return 'function' == typeof H[H.length - 1] && (F('emitting packet with ack id %d', this.ids), this.acks[this.ids] = H.pop(), W.id = this.ids++), this.connected ? this.packet(W) : this.sendBuffer.push(W), this
				...
				F('close (%s)', q), this.connected = !1, this.disconnected = !0, delete this.id, this.emit('disconnect', q)
				...
				var H = q.data || [];
				F('emitting event %j', H), null != q.id && (F('attaching ack callback to event'), H.push(this.ack(q.id))), this.connected ? V.apply(this, H) : this.receiveBuffer.push(H)
				...
				H.apply(this, q.data), delete this.acks[q.id]
		...
		65: [function(P, R) {
			(function(M) {
				...
				R.exports = function(N, U) {
					var F = N,
						U = U || M.location;
					return null == N && (N = U.protocol + '//' + U.host), 'string' == typeof N && ('/' == N.charAt(0) && ('/' == N.charAt(1) ? N = U.protocol + N : N = U.hostname + N), !/^(https?|wss?):\/\//.test(N) && (B('protocol-less url %s', N), N = 'undefined' == typeof U ? 'https://' + N : U.protocol + '//' + N), B('parse %s', N), F = I(N)), F.port || (/^(http|ws)$/.test(F.protocol) ? F.port = '80' : /^(http|ws)s$/.test(F.protocol) && (F.port = '443')), F.path = F.path || '/', F.id = F.protocol + '://' + F.host + ':' + F.port, F.href = F.protocol + '://' + F.host + (U && U.port == F.port ? '' : ':' + F.port), F
				}
			}).call(this, 'undefined' == typeof global ? 'undefined' == typeof self ? 'undefined' == typeof window ? {} : window : self : global)
		}, {
			debug: 19,
			...
			(function(M) {
				...
				D.deconstructPacket = function(B) {
					...
					var U = [],
						F = B.data,
						G = B;
					return G.data = N(F), G.attachments = U.length, {
						packet: G,
						buffers: U
					}
					...
					return B.data = U(B.data), B.attachments = void 0, B
					...
					function U(z, V, q) {
						...
						if (M.Blob && z instanceof Blob || M.File && z instanceof File) {
							...
							H.onload = function() {
								q ? q[V] = this.result : G = this.result, --F || N(G)
		...
		67: [function(P, R, D) {
			...
			function A(W) {
				var X = '',
					Y = !1;
				return X += W.type, (D.BINARY_EVENT == W.type || D.BINARY_ACK == W.type) && (X += W.attachments, X += '-'), W.nsp && '/' != W.nsp && (Y = !0, X += W.nsp), null != W.id && (Y && (X += ',', Y = !1), X += W.id), null != W.data && (Y && (X += ','), X += z.stringify(W.data)), G('encoded %j as %s', W, X), X
			}
			...
			var G = P('debug')('socket.io-parser'),
				...
				H = P('./binary'),
				J = P('./is-buffer');
			D.protocol = 4, D.types = ['CONNECT', 'DISCONNECT', 'EVENT', 'BINARY_EVENT', 'ACK', 'BINARY_ACK', 'ERROR'], D.CONNECT = 0, D.DISCONNECT = 1, D.EVENT = 2, D.ACK = 3, D.ERROR = 4, D.BINARY_EVENT = 5, D.BINARY_ACK = 6, D.Encoder = M, D.Decoder = B, M.prototype.encode = function(W, X) {
				if (G('encoding packet %j', W), D.BINARY_EVENT == W.type || D.BINARY_ACK == W.type) I(W, X);
				else {
					var Y = A(W);
					X([Y])
				}
			}, q(B.prototype), B.prototype.add = function(W) {
				var X;
				if ('string' == typeof W) X = N(W), D.BINARY_EVENT == X.type || D.BINARY_ACK == X.type ? (this.reconstructor = new U(X), 0 === this.reconstructor.reconPack.attachments && this.emit('decoded', X)) : this.emit('decoded', X);
				...
				else X = this.reconstructor.takeBinaryData(W), X && (this.reconstructor = null, this.emit('decoded', X))
				...
				if (this.buffers.push(W), this.buffers.length == this.reconPack.attachments) {
					...
					return this.finishedReconstruction(), X
				}
				return null
		...
		70: [function(P, R, D) {
			(function(M) {
				(function(A) {
					function I(ie) {
						for (var se = [], de = 0, ce = ie.length, pe, le; de < ce;) pe = ie.charCodeAt(de++), 55296 <= pe && 56319 >= pe && de < ce ? (le = ie.charCodeAt(de++), 56320 == (64512 & le) ? se.push(((1023 & pe) << 10) + (1023 & le) + 65536) : (se.push(pe), de--)) : se.push(pe);
						return se
					}

					function B(ie) {
						for (var se = ie.length, de = -1, pe = '', ce; ++de < se;) ce = ie[de], 65535 < ce && (ce -= 65536, pe += X(55296 | 1023 & ce >>> 10), ce = 56320 | 1023 & ce), pe += X(ce);
						return pe
					}
					...
					function F(ie) {
						...
						var se = '';
						return 0 == (4294965248 & ie) ? se = X(192 | 31 & ie >> 6) : 0 == (4294901760 & ie) ? (N(ie), se = X(224 | 15 & ie >> 12), se += U(ie, 6)) : 0 == (4292870144 & ie) && (se = X(240 | 7 & ie >> 18), se += U(ie, 12), se += U(ie, 6)), se += X(128 | 63 & ie), se
					}
					...
					function V() {
						...
						if (224 == (240 & ie)) {
							if (se = z(), de = z(), pe = (15 & ie) << 12 | se << 6 | de, 2048 <= pe) return N(pe), pe;
							throw Error('Invalid continuation byte')
						}
					...
					var X = String.fromCharCode,
						Y, K, Z, Q = {
							version: '2.0.0',
							encode: function(ie) {
								for (var se = I(ie), de = se.length, ce = -1, le = '', pe; ++ce < de;) pe = se[ce], le += F(pe);
								return le
							},
							decode: function(ie) {
								Y = I(ie), K = Y.length, Z = 0;
		...
		71: [function(P, R) {
			R.exports = 'function' == typeof Object.create ? function(A, I) {
				A.super_ = I, A.prototype = Object.create(I.prototype, {
					constructor: {
						value: A,
						enumerable: !1,
						writable: !0,
				...
				A.super_ = I;
				var B = function() {};
				B.prototype = I.prototype, A.prototype = new B, A.prototype.constructor = A
			}
		}, {}],
		72: [function(P, R) {
			R.exports = function(A) {
		...
		73: [function(P, R, D) {
			(function(M, A) {
				function I(be, ve) {
					...
					return 3 <= arguments.length && (ke.depth = arguments[2]), 4 <= arguments.length && (ke.colors = arguments[3]), W(ve) ? ke.showHidden = ve : ve && D._extend(ke, ve), ne(ke.showHidden) && (ke.showHidden = !1), ne(ke.depth) && (ke.depth = 2), ne(ke.colors) && (ke.colors = !1), ne(ke.customInspect) && (ke.customInspect = !0), ke.colors && (ke.stylize = B), F(ke, be, ke.depth)
				}
				...
				function q(be, ve, ke, Ce, Se, xe) {
					...
					if (Ee = Object.getOwnPropertyDescriptor(ve, Se) || {
						...
						Te = JSON.stringify('' + Se), Te.match(/^"([a-zA-Z_][a-zA-Z_0-9]*)"$/) ? (Te = Te.substr(1, Te.length - 2), Te = be.stylize(Te, 'name')) : (Te = Te.replace(/'/g, '\\\'').replace(/\\"/g, '"').replace(/(^"|"$)/g, '\''), Te = be.stylize(Te, 'string'))
					}
					return Te + ': ' + we
				}
				...
				function H(be, ve, ke) {
					var Ce = 0,
						Se = be.reduce(function(xe, Te) {
							return Ce++, 0 <= Te.indexOf('\n') && Ce++, xe + Te.replace(/\u001b\[\d\d?m/g, '').length + 1
						}, 0);
				...
				D.debuglog = function(be) {
					...
					date: 'magenta',
					regexp: 'red'
				}, D.isArray = J, D.isBoolean = W, D.isNull = X, D.isNullOrUndefined = function(be) {
					return null == be
				}, D.isNumber = K, D.isString = Z, D.isSymbol = function(be) {
					return 'symbol' == typeof be
				}, D.isUndefined = ne, D.isRegExp = ae, D.isObject = oe, D.isDate = ie, D.isError = se, D.isFunction = de, D.isPrimitive = function(be) {
				...
				}, D.inherits = P('inherits'), D._extend = function(be, ve) {
		...
		75: [function(P, R) {
			function M() {}
			R.exports = M, M.mixin = function(A) {
				var I = A.prototype || A;
				I.isWildEmitter = !0, I.on = function(B) {
					this.callbacks = this.callbacks || {};
					var F = 3 === arguments.length,
						G = F ? arguments[1] : void 0,
						z = F ? arguments[2] : arguments[1];
					return z._groupName = G, (this.callbacks[B] = this.callbacks[B] || []).push(z), this
				}, I.once = function(B) {
					function F() {
						G.off(B, F), q.apply(this, arguments)
					}
					var G = this,
						z = 3 === arguments.length,
						V = z ? arguments[1] : void 0,
						q = z ? arguments[2] : arguments[1];
					return this.on(B, V, F), this
		...
		76: [function(P, R) {
			function M(B, N) {
				var F;
				return F = N ? new I(B, N) : new I(B), F
			}
			var A = function() {
					return this
				}(),
				I = A.WebSocket || A.MozWebSocket;
			R.exports = I ? M : null, I && (M.prototype = I.prototype)
		}, {}],
		77: [function(P, R) {
			function M(z) {
				var V = !0;
			...
			function A(z) {
				var V = this;
				U.call(this), this.id = z.id, this.parent = z.parent, this.type = z.type || 'video', this.oneway = z.oneway || !1, this.sharemyscreen = z.sharemyscreen || !1, this.browserPrefix = z.prefix, this.stream = z.stream, this.enableDataChannels = z.enableDataChannels === void 0 ? this.parent.config.enableDataChannels : z.enableDataChannels, this.receiveMedia = z.receiveMedia || this.parent.config.receiveMedia, this.channels = {}, this.sid = z.sid || Date.now().toString(), this.pc = new N(this.parent.config.peerConnectionConfig, this.parent.config.peerConnectionConstraints), this.pc.on('ice', this.onIceCandidate.bind(this)), this.pc.on('endOfCandidates', function(q) {
					V.send('endOfCandidates', q)
				}), this.pc.on('offer', function(q) {
					V.parent.config.nick && (q.nick = V.parent.config.nick), V.send('offer', q)
				}), this.pc.on('answer', function(q) {
					V.parent.config.nick && (q.nick = V.parent.config.nick), V.send('answer', q)
				...
				}), this.pc.on('signalingStateChange', this.emit.bind(this, 'signalingStateChange')), this.logger = this.parent.logger, 'screen' === z.type ? this.parent.localScreens && this.parent.localScreens[0] && this.sharemyscreen && (this.logger.log('adding local screen stream to peer connection'), this.pc.addStream(this.parent.localScreens[0]), this.broadcaster = z.broadcaster) : this.parent.localStreams.forEach(function(q) {
					V.pc.addStream(q)
				}), this.on('channelOpen', function(q) {
					q.protocol === G && (q.onmessage = function(H) {
						var J = JSON.parse(H.data),
							W = new F.Receiver;
						W.receive(J, q), V.emit('fileTransfer', J, W), W.on('receivedFile', function() {
							W.channel.close()
						})
					})
				}), this.on('*', function() {
			...
			I.inherits(A, U), A.prototype.handleMessage = function(z) {
				var V = this;
				if (this.logger.log('getting', z.type, z), z.prefix && (this.browserPrefix = z.prefix), 'offer' === z.type) this.nick || (this.nick = z.payload.nick), delete z.payload.nick, this.pc.handleOffer(z.payload, function(H) {
					H || V.pc.answer(function() {})
				});
				else if ('answer' === z.type) this.nick || (this.nick = z.payload.nick), delete z.payload.nick, this.pc.handleAnswer(z.payload);
				...
				this.logger.log('sending', z, q), this.parent.emit('message', q)
				...
				var V = this;
				z.onclose = this.emit.bind(this, 'channelClose', z), z.onerror = this.emit.bind(this, 'channelError', z), z.onmessage = function(q) {
				...
				z.mandatory.IceRestart = !0, this.pc.offer(z, function() {})
				...
				this.channels[z.label] = z, this._observeDataChannel(z)
				...
				return q.onopen = function() {
					...
					console.log('sender received transfer'), V.emit('complete')
				}, V
			}, R.exports = A
		}, {
			filetransfer: 35,
		...
		78: [function(P, R) {
			function M(G) {
				...
				this.capabilities = B, I.call(this), J = null === this.config.connection ? this.connection = new F(this.config) : this.connection = this.config.connection, J.on('connect', function() {
					z.emit('connectionReady', J.getSessionid()), z.sessionReady = !0, z.testReadiness()
				...
				}), G.logger = this.logger, G.debug = !1, this.webrtc = new A(G), ['mute', 'unmute', 'pauseVideo', 'resumeVideo', 'pause', 'resume', 'sendToAll', 'sendDirectlyToAll', 'getPeers'].forEach(function(W) {
					z[W] = z.webrtc[W].bind(z.webrtc)
				}), this.webrtc.on('*', function() {
					z.emit.apply(z, arguments)
				}), q.debug && this.on('*', this.logger.log.bind(this.logger, 'SimpleWebRTC event:')), this.webrtc.on('localStream', function() {
					z.testReadiness()
				}), this.webrtc.on('message', function(W) {
					z.connection.emit('message', W)
				}), this.webrtc.on('peerStreamAdded', this.handlePeerStreamAdded.bind(this)), this.webrtc.on('peerStreamRemoved', this.handlePeerStreamRemoved.bind(this)), this.config.adjustPeerVolume && (this.webrtc.on('speaking', this.setVolumeForAll.bind(this, this.config.peerVolumeWhenSpeaking)), this.webrtc.on('stoppedSpeaking', this.setVolumeForAll.bind(this, 1))), J.on('stunservers', function(W) {
					z.webrtc.config.peerConnectionConfig.iceServers = W, z.emit('stunservers', W)
				}), J.on('turnservers', function(W) {
					z.webrtc.config.peerConnectionConfig.iceServers = z.webrtc.config.peerConnectionConfig.iceServers.concat(W), z.emit('turnservers', W)
					...
					Y.oncontextmenu = function() {
						return !1
					}, Y.id = 'localScreen', N(W, Y), K && K.appendChild(Y), z.emit('localScreenAdded', Y), z.connection.emit('shareScreen'), z.webrtc.peers.forEach(function(Z) {
						var Q;
			...
			M.prototype = Object.create(I.prototype, {
				...
				var z = this,
					...
					q = N(G.stream);
				G.videoEl = q, q.id = this.getDomId(G), V && V.appendChild(q), this.emit('videoAdded', q, G), window.setTimeout(function() {
				...
				var z = this.getRemoteVideoContainer(),
					V = G.videoEl;
				this.config.autoRemoveVideos && z && V && z.removeChild(V), V && this.emit('videoRemoved', V, G)
				...
				var V = this;
				this.roomName = G, this.connection.emit('join', G, function(q, H) {
				...
				this.config.autoRemoveVideos && z && G && z.removeChild(G), G && this.emit('videoRemoved', G), this.getLocalScreen() && this.webrtc.stopScreenShare(), this.webrtc.peers.forEach(function(V) {
					V.broadcaster && V.end()
				})
				...
				this.roomName = G, 2 === arguments.length ? this.connection.emit('create', G, z) : this.connection.emit('create', G)
		...
		80: [function(P, R) {
			function M(F) {
				...
				I.support || this.logger.error('Your browser doesn\'t seem to support WebRTC'), this.peers = [], N.call(this, this.config), this.on('speaking', function() {
					...
					var X;
					X = G.config.logger === B ? console : G.logger, X.log('event:', H, J, W)
				})
			}
			var A = P('util'),
				I = P('webrtcsupport'),
				...
				N = P('localmedia'),
				U = P('./peer');
			A.inherits(M, N), M.prototype.createPeer = function(F) {
				var G;
				return F.parent = this, G = new U(F), this.peers.push(G), G
